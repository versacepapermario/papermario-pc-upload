...
void gfx_task_background(void) {
#ifdef PORT
    // Skip background DL when drawing is disabled (loading screens, map transitions).
    // gfx_draw_frame() will also skip, so this background DL would never be consumed
    // and would become stale garbage data.
    ...
    ASSERT((gMainGfxPos - gDisplayContext->backgroundGfx) < (s32)ARRAY_COUNT(gDisplayContext->backgroundGfx))
...
void gfx_draw_frame(void) {
...
#ifdef PORT
    // PORT: Enable all render paths EXCEPT render_frame (needs texture endian fixes)
    // and render_hud_elements (needs ROM texture loading).
    // Follows original N64 render order.
    ...
    ASSERT((gMainGfxPos - gDisplayContext->mainGfx) < (s32)ARRAY_COUNT(gDisplayContext->mainGfx));
