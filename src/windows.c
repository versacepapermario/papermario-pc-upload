...
void render_windows(s32* windowsArray, s32 parent, s32 flags, s32 baseX, s32 baseY, s32 opacity, s32 darkening, f32 (*rotScaleMtx)[4]) {
    ...
    for (i = 0, windowArrayIt = windowsArray; i < ARRAY_COUNT(gWindows); i++, windowArrayIt++) {
        ...
        childFlags |= flags;
#ifdef PORT
        // PORT: N64 skips baseX/baseY when ROTSCALE is set because it renders
        // to an offscreen buffer, then composites at the right position later.
        // PC port rotates in-place, so always needs absolute screen position.
        posX += baseX;
        posY += baseY;
#else
        if (!(childFlags & DRAW_FLAG_ROTSCALE)) {
            posX += baseX;
            posY += baseY;
        }
#endif
