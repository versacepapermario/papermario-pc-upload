...
#ifdef PORT
            // PORT: Skip VISCVG coverage extraction (requires G_BL_CLR_BL and framebuffer access).
            // Go directly to SHOW_PAUSED with a dark background instead of a freeze-frame.
...
#ifdef PORT
            // PORT: Skip CPU-side framebuffer filtering. Go to SHOW_PAUSED.
            gPauseBackgroundFade = 0;
            gGameStatusPtr->backgroundFlags &= ~BACKGROUND_RENDER_STATE_MASK;
            gGameStatusPtr->backgroundFlags |= BACKGROUND_RENDER_STATE_SHOW_PAUSED;
            // fallthrough
...
#ifdef PORT
            // PORT: Skip loading nuGfxZBuffer as texture — it's not the GPU framebuffer.
            // The dark fill from above serves as the pause background.
#else
            gDPSetCycleType(gMainGfxPos++, G_CYC_1CYCLE);
            gDPSetTexturePersp(gMainGfxPos++, G_TP_NONE);
            gDPSetTextureLUT(gMainGfxPos++, G_TT_NONE);
