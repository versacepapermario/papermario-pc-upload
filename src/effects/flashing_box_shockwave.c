...
#ifndef PORT
    // PORT: Skip the box flash overlay. On N64, ZMODE_DEC + Z_CMP clips this quad to
    // only render where enemy geometry exists (depth-range test). On PC, polygon offset
    // can't replicate this, so the full rectangle would be visible as a flash box.
