...
void waterfall_appendGfx(void* effect) {
    ...
    // 24 vertices inline in DL. N64 Vtx=16 bytes→48 Gfx; PORT Vtx=24 bytes→72 Gfx.
#ifdef GBI_FLOATS
    #define WATERFALL_VTX_GFX_COUNT 72
#else
    #define WATERFALL_VTX_GFX_COUNT 48
#endif
    gSPBranchList(gMainGfxPos, &gMainGfxPos[WATERFALL_VTX_GFX_COUNT + 1]);

    savedGfxPos = gMainGfxPos + 1;
    gMainGfxPos = &gMainGfxPos[WATERFALL_VTX_GFX_COUNT + 1];
