...
void chomp_drop_appendGfx(void* effect) {
    ...
    if (data->unk_2C == 1) {
        ...
        // Inline vertex trick: write vertex data into the display list buffer.
        // Must account for Vtx_t size: 16 bytes on N64, 24 bytes with GBI_FLOATS.
        // 32 vertices needed. Gfx entry = 8 bytes.
        {
#ifdef GBI_FLOATS
            // 32 vtx × 24 bytes = 768 bytes = 96 Gfx entries (0x60)
            #define CHOMP_VTX_GFX_COUNT 0x60
#else
            // 32 vtx × 16 bytes = 512 bytes = 64 Gfx entries (0x40)
            #define CHOMP_VTX_GFX_COUNT 0x40
#endif
            savedGfxPos = gMainGfxPos + 1;
            gSPBranchList(gMainGfxPos, gMainGfxPos + CHOMP_VTX_GFX_COUNT + 1);
        ...
        gMainGfxPos += CHOMP_VTX_GFX_COUNT;
