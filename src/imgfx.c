...
#ifdef PORT
// On PC, N64 linker symbols don't hold offset values. Use actual offsets from splat.yaml.
// These are byte offsets within the imgfx_data ROM segment to each animation's ImgFXAnimHeader.
u32 ImgFXAnimOffsets[] = {
...
void imgfx_update(u32 idx, ImgFXType type, s32 imgfxArg1, s32 imgfxArg2, s32 imgfxArg3, s32 imgfxArg4, s32 flags) {
    ...
    if (type == IMGFX_SET_WAVY || type == IMGFX_SET_ANIM) {
        ...
        if (type == IMGFX_OVERLAY || type == IMGFX_OVERLAY_XLU) {
            // PORT: overlay.pattern is a pointer (8 bytes on 64-bit), must store through
            // the union's overlay member, not the s32 color array which would truncate
...
ImgFXAnimHeader* imgfx_load_anim(ImgFXState* state) {
#ifdef PORT
    u8* romStart = imgfx_data_ROM_START + ImgFXAnimOffsets[state->ints.anim.type];
#else
    ...
    if (state->curAnimOffset != romStart) {
...
#ifdef PORT
        // Byte-swap big-endian ROM data
        anim->keyframesOffset = __builtin_bswap32(anim->keyframesOffset);
        anim->gfxOffset = __builtin_bswap32(anim->gfxOffset);
        anim->vtxCount = __builtin_bswap16(anim->vtxCount);
        anim->gfxCount = __builtin_bswap16(anim->gfxCount);
        anim->keyframesCount = __builtin_bswap16(anim->keyframesCount);
        anim->flags = __builtin_bswap16(anim->flags);
#endif
...
#ifdef PORT
        // N64 Gfx = 8 bytes (two u32), PC Gfx = 16 bytes (two uintptr_t).
        // DMA only the N64-sized data, then widen in-place.
        {
            u32 n64GfxSize = anim->gfxCount * 8; // N64 sizeof(Gfx) = 8
            romStart = imgfx_data_ROM_START + (u32)anim->gfxOffset;
            romEnd = romStart + n64GfxSize;

            for (i = 0; i < ARRAY_COUNT(state->gfxBufs); i++) {
                // DMA N64-sized data into start of PC-sized buffer
                dma_copy(romStart, romEnd, state->gfxBufs[i]);

                // Convert N64 8-byte Gfx to PC 16-byte Gfx, working backwards to avoid overlap
                u32* n64Words = (u32*)state->gfxBufs[i];
                Gfx* pcGfx = (Gfx*)state->gfxBufs[i];
                for (s32 k = anim->gfxCount - 1; k >= 0; k--) {
                    u32 w0 = __builtin_bswap32(n64Words[k * 2 + 0]);
                    u32 w1 = __builtin_bswap32(n64Words[k * 2 + 1]);
                    pcGfx[k].words.w0 = (uintptr_t)w0;
                    pcGfx[k].words.w1 = (uintptr_t)w1;
                }
            }
        }
#else
        ...
        for (i = 0; i < ARRAY_COUNT(state->gfxBufs); i++) {
            ...
            do {
                ...
                if (cmd == G_VTX) {
                    // ImgFXVtx structs are 0xC bytes. Convert byte offset in ImgFXVtx space
                    // to byte offset in Vtx space (sizeof(Vtx) varies: 0x10 normal, 0x18 with GBI_FLOATS).
#ifdef PORT
                    gfxBuffer[j-1].words.w1 = ((((u32) gfxBuffer[j-1].words.w1 - (u32) anim->keyframesOffset) / sizeof(ImgFXVtx)) * sizeof(Vtx)) +
                                              (uintptr_t) state->vtxBufs[i];
#else
                    // Unfortunately, using sizeof here does not match.
                    gfxBuffer[j-1].words.w1 = ((((u32) gfxBuffer[j-1].words.w1 - (u32) anim->keyframesOffset) / 3) * 4) +
                                              (uintptr_t) state->vtxBufs[i];
#endif
                }
            } while (cmd != G_ENDDL);
...
void imgfx_mesh_anim_update(ImgFXState* state) {
    ...
    romStart = imgfx_data_ROM_START + (u32) header->keyframesOffset + curKeyIdx * header->vtxCount * sizeof(ImgFXVtx);
    dma_copy(romStart, romStart + header->vtxCount * sizeof(ImgFXVtx), curKeyframe);
#ifdef PORT
    // Byte-swap s16 ob[] fields from big-endian ROM
    for (i = 0; i < header->vtxCount; i++) {
        curKeyframe[i].ob[0] = __builtin_bswap16(curKeyframe[i].ob[0]);
        curKeyframe[i].ob[1] = __builtin_bswap16(curKeyframe[i].ob[1]);
        curKeyframe[i].ob[2] = __builtin_bswap16(curKeyframe[i].ob[2]);
    }
#endif
    if (keyframeInterval > 1) {
        nextKeyframe = heap_malloc(header->vtxCount * sizeof(*nextKeyframe));
        romStart = imgfx_data_ROM_START + (u32) header->keyframesOffset + nextKeyIdx * header->vtxCount * sizeof(ImgFXVtx);
        dma_copy(romStart, romStart + header->vtxCount * sizeof(ImgFXVtx), nextKeyframe);
#ifdef PORT
        for (i = 0; i < header->vtxCount; i++) {
            nextKeyframe[i].ob[0] = __builtin_bswap16(nextKeyframe[i].ob[0]);
            nextKeyframe[i].ob[1] = __builtin_bswap16(nextKeyframe[i].ob[1]);
            nextKeyframe[i].ob[2] = __builtin_bswap16(nextKeyframe[i].ob[2]);
        }
#endif
    }
    ...
    for (i = 0; i < header->vtxCount; i++) {
        ...
        if (keyframeInterval > 1) {
            ...
            if (state->flags & (IMGFX_FLAG_2000 | IMGFX_FLAG_8000)) {
                ...
                state->vtxBufs[gCurrentDisplayContextIndex][i].v.cn[2] = (u8)(s32)(240.0 - (curKeyframe[i].tc[0] + curKeyframe[i].tc[1]) * 1.2);
            }
        } else {
            // get vertex position
            if (header->flags & IMGFX_ANIM_FLAG_ABSOLUTE_COORDS) {
