...
API_CALLABLE(N(SkateInCirclePenguin1)) {
    ...
    if (isInitialCall) {
        ...
        (*(f32*)&script->functionTemp[3]) = 180.0f;
        script->functionTemp[0] = 72;
    }

    npc = script->functionTempPtr[1];
    (*(f32*)&script->functionTemp[3]) = clamp_angle((*(f32*)&script->functionTemp[3]) - 5.0f);
    npc->pos.x = npc->pos.x + (cos_deg((*(f32*)&script->functionTemp[3])) * 6.0f);
    npc->pos.z = npc->pos.z + (sin_deg((*(f32*)&script->functionTemp[3])) * 2.5f);
    npc->renderYaw = clamp_angle(180.0f - (*(f32*)&script->functionTemp[3]));
