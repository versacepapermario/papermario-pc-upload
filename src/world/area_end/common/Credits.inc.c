...
void N(credits_update_line)(CreditsLine* line) {
    ...
    get_msg_properties((intptr_t) line->message, &msgHeight, &msgWidth, &msgMaxLineChars, nullptr, nullptr, nullptr, 0);
    ...
    if ((line->state == CREDITS_LINE_APPEARING) && doneCurrentState) {
        s32 temp = 0;
        get_msg_properties((intptr_t) line->message, nullptr, nullptr, &temp, nullptr, nullptr, nullptr, 0);
        line->time = 0;
        line->state++;
        if (line->holdTime <= 0) {
            line->state++;
...
void N(credits_load_message)(CreditsEntry* entry) {
    ...
    if (entry->msgID != MSG_NONE) {
#ifdef PORT
        if ((uintptr_t)entry->msgID <= 0xFFFFFF) {
#else
        if (entry->msgID >= 0) {
#endif
        ...
        get_msg_properties((intptr_t) line->message, nullptr, nullptr, &maxLineChars, nullptr, nullptr, &numSpaces, 0);
...
API_CALLABLE(N(ShowCreditList)) {
    CreditsEntry* creditList = (CreditsEntry*)(intptr_t) evt_get_variable(script, *script->ptrReadPos);
