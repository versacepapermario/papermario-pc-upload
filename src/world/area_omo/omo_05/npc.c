...
API_CALLABLE(N(SpinCameraAround)) {
    ...
    if (isInitialCall) {
        ...
        (*(f32*)&script->functionTemp[0]) = 0.0f;
    }
    (*(f32*)&script->functionTemp[0]) += 10.0f;
    guRotateF(camera->mtxViewShaking, (*(f32*)&script->functionTemp[0]), 0.0f, 0.0f, -1.0f);

    if ((*(f32*)&script->functionTemp[0]) >= 360.0) {
        return ApiStatus_DONE2;
    } else {
