...
#ifdef PORT
// PORT: BufRead reads 4 bytes per entry, but pointer arrays have 8-byte entries on 64-bit.
// This helper reads the Nth pointer from a pointer array correctly.
API_CALLABLE(N(ReadPtrArrayEntry)) {
    Bytecode* args = script->ptrReadPos;
    void** array = (void**)(intptr_t)evt_get_variable(script, *args++);
    s32 index = evt_get_variable(script, *args++);
    Bytecode outVar = *args++;
    evt_set_variable(script, outVar, (Bytecode)(intptr_t)array[index]);
...
#ifdef PORT
                Sub(LVar0, 1)
                Call(N(ReadPtrArrayEntry), Ref(N(CrowdFleeScripts)), LVar0, LVar1)
#else
