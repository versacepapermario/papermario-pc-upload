...
API_CALLABLE(N(SurroundPlayer)) {
    ...
    if ((*(f32*)&script->varTable[11]) == playerStatus->pos.x && (*(f32*)&script->varTable[13]) == playerStatus->pos.z) {
        if (dist < 20.0f) {
            script->varTable[14]++;
        } else {
    ...
    (*(f32*)&script->varTable[11]) = playerStatus->pos.x;
    (*(f32*)&script->varTable[12]) = playerStatus->pos.y;
    (*(f32*)&script->varTable[13]) = playerStatus->pos.z;
