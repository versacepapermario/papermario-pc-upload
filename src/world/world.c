...
#ifndef PORT
        // On PC, map overlay code is statically linked — skip DMA to VRAM stub
...
#ifdef PORT
        // N64 shape data has 32-bit big-endian pointers at 0x80210000 base.
        // Rebuild model tree with proper 64-bit PC pointers.
        {
            ModelNode* root;
            Vtx_t* vtxTable;
            char** modelNames;
            char** colliderNames;
            char** zoneNames;
            shape_data_swizzle_n64_to_pc(
                shapeFile, decompressedSize,
                &root, &vtxTable, &modelNames, &colliderNames, &zoneNames
            );
            mapSettings->modelTreeRoot = root;
            mapSettings->modelNameList = modelNames;
            mapSettings->colliderNameList = colliderNames;
            mapSettings->zoneNameList = zoneNames;
        }
#else
...
#ifdef PORT
    // PORT: Clear startup fade screen overlay. The logo state sets this to gray (224,224,224)
    // with alpha=255. If not explicitly cleared, it persists as a faint tint, making black
    // out-of-bounds areas appear gray instead of black.
...
#ifdef PORT
static void asset_header_bswap(AssetHeader* h) {
    h->offset = ROM_BSWAP32(h->offset);
    h->compressedLength = ROM_BSWAP32(h->compressedLength);
    h->decompressedLength = ROM_BSWAP32(h->decompressedLength);
}

static void asset_table_bswap(AssetHeader* table, u32 byteSize) {
    u32 count = byteSize / sizeof(AssetHeader);
    for (u32 i = 0; i < count; i++) {
        asset_header_bswap(&table[i]);
    }
}
#endif
...
#ifdef PORT
    asset_header_bswap(&firstHeader);
#endif
...
#ifdef PORT
    asset_table_bswap(assetTableBuffer, firstHeader.offset);
#endif
    {
#ifdef PORT
        ...
        while (strcmp(curAsset->name, assetName) != 0) {
            ...
            idx++;
            if (idx >= numEntries) {
                fprintf(stderr, "[asset] ERROR: '%s' not found in asset table (%u entries)\n", assetName, numEntries);
