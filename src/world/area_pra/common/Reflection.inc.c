...
API_CALLABLE(N(EnableWallReflection)){
    port_array_set(script->array, 0, create_worker_scene(nullptr, N(worker_reflect_player_wall)));
...
API_CALLABLE(N(EnableFloorReflection)){
    switch (script->varTable[0]) {
        ...
        case REFLECTION_FLOOR_ONLY:
            port_array_set(script->array, 0, create_worker_scene(nullptr, N(worker_reflect_player_floor)));
...
API_CALLABLE(N(EnablePartnerReflection)){
    ...
    if (script->varTable[1] == false) {
        switch (script->varTable[0]) {
            case REFLECTION_FLOOR_WALL:
                port_array_set(script->array, 1, create_worker_scene(N(worker_reflect_partner_all), nullptr));
                break;
            case REFLECTION_FLOOR_ONLY:
                port_array_set(script->array, 1, create_worker_scene(N(worker_reflect_partner_floor), nullptr));
                break;
            case REFLECTION_WALL_ONLY:
                port_array_set(script->array, 1, create_worker_scene(N(worker_reflect_partner_wall), nullptr));
                break;
        }
    } else {
        switch (script->varTable[0]) {
