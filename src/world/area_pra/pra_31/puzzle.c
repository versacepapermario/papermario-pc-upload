...
API_CALLABLE(N(EVS_ManagePuzzle)) {
    ...
    if (isInitialCall) {
        ...
        evt_set_variable(script, MV_PuzzleDataPtr, (intptr_t) puzzle);
    ...
    puzzle = (DinoPuzzleData*)(intptr_t) evt_get_variable(script, MV_PuzzleDataPtr);
    dino = &puzzle->dinos[0];
...
API_CALLABLE(N(SetDinoAngle)) {
    ...
    DinoPuzzleData* puzzle = (DinoPuzzleData*)(intptr_t) evt_get_variable(script, MV_PuzzleDataPtr);
