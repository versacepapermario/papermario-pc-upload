...
#define PIRANHA_DMA_ENTRY(name) \
    (intptr_t) world_model_anim_kzn_##name##_ROM_START + DEAD_ROM_OFFSET,\
    (intptr_t) world_model_anim_kzn_##name##_ROM_END + DEAD_ROM_OFFSET,\
...
API_CALLABLE(N(SetVineBonePos)) {
    ...
    LavaPiranhaVine* vines = (LavaPiranhaVine*)(intptr_t) evt_get_variable(nullptr, MV_VinesData);
...
API_CALLABLE(N(CreateVineRenderer)) {
    ...
    evt_set_variable(script, MV_VinesData, (intptr_t) data);
    N(VineRenderState) = -1;
