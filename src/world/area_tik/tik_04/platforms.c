...
API_CALLABLE(N(CreatePlatformShadows)) {
    ...
    port_array_set(script->array, 0, create_shadow_type(SHADOW_VARYING_SQUARE, model->center.x, model->center.y - 100.0f, model->center.z));
    model = get_model_from_list_index(get_model_list_index_from_tree_index(MODEL_erb2));
    port_array_set(script->array, 1, create_shadow_type(SHADOW_VARYING_SQUARE, model->center.x, model->center.y - 100.0f, model->center.z));
...
API_CALLABLE(N(UpdatePlatformShadows)) {
    ...
    shadow = get_shadow_by_index(port_array_get(script->array, 0));
    model = get_model_from_list_index(get_model_list_index_from_tree_index(MODEL_erb1));
    x = model->center.x;
    y = model->center.y;
    z = model->center.z;
    ...
    shadow = get_shadow_by_index(port_array_get(script->array, 1));
    model = get_model_from_list_index(get_model_list_index_from_tree_index(MODEL_erb2));
    x = model->center.x;
    y = model->center.y;
    z = model->center.z;
