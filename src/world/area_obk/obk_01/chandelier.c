...
API_CALLABLE(N(UpdateChandelier)) {
    ...
    if (isInitialCall) {
        ...
        chandelier->controlData = (ChandelierControlData*)(intptr_t) evt_get_variable(script, *args++);
...
API_CALLABLE(N(ChandelierTryCancel)) {
    ...
    if (isInitialCall) {
        ChandelierControlData* data = (ChandelierControlData*)(intptr_t) evt_get_variable(script, *args++);
