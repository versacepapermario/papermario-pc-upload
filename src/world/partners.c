...
#ifdef PORT
    // On N64, freed heap memory remained accessible; on 64-bit PC it's invalid.
    // NULL the global and any active partner script's cached NPC pointer.
    wPartnerNpc = NULL;
    if (wPartnerCurrentScript != NULL) {
        wPartnerCurrentScript->owner2.npc = NULL;
    }
#endif
}
