...
void N(animate_and_draw_record)(void* renderData) {
    RecordDisplayData* data = (RecordDisplayData*)(intptr_t)evt_get_variable(nullptr, MV_RecordDisplayData);
...
API_CALLABLE(N(UpdateRecordDisplay)) {
    ...
    if (isInitialCall) {
        ...
        evt_set_variable(script, MV_RecordDisplayData, (intptr_t)data);
    }
