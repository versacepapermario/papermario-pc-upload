...
API_CALLABLE(N(SetStarSpiritSparkleTrailPos)) {
    Bytecode* args = script->ptrReadPos;
    EffectInstance* effect = (EffectInstance*)(intptr_t) evt_get_variable(script, *args++);
...
API_CALLABLE(N(SetCardCaptureState1)) {
    Bytecode* args = script->ptrReadPos;
    EffectInstance* effect = (EffectInstance*)(intptr_t) evt_get_variable(script, ArrayVar(0));
...
API_CALLABLE(N(SetLightRayPos)) {
    Bytecode* args = script->ptrReadPos;
    EffectInstance* effect = (EffectInstance*)(intptr_t) evt_get_variable(script, ArrayVar(16));
...
API_CALLABLE(func_802428C8_A2CB08) {
    ...
    arrayVar0 = (EffectInstance*)(intptr_t) evt_get_variable(script, ArrayVar(0));
    effect = arrayVar0;
...
void N(worker_draw_story_graphics)(void) {
    ...
    if (N(StoryGraphicsPtr)->storyPageAlpha < 255) {
        ...
        // Ensure PRIMITIVE alpha is set even when storyPageAlpha == 255.
        // On N64, the register persists from prior state. On PC/Fast3D it may default to 0.
...
void N(load_story_image)(s32 loadBackImage, s32 imageIdx) {
    ...
    if (imageIdx < 0) {
        ...
        {
#ifdef PORT
            // Fast3D expects palette data in N64 big-endian format.
            // Runtime-generated u16 values are native little-endian on PC, so byte-swap.
