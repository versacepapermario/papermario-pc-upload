...
void N(gfx_build_knocked_down_player)(void) {
    KnockdownData* data = (KnockdownData*)(intptr_t) evt_get_variable(nullptr, KNOCK_DOWN_MAP_VAR);
    ImgFXTexture ifxImg;
...
API_CALLABLE(N(KnockdownCreate)) {
    ...
    evt_set_variable(script, KNOCK_DOWN_MAP_VAR, (intptr_t) data);
...
API_CALLABLE(N(KnockdownDestroy)) {
    ...
    heap_free(data);
    evt_set_variable(script, KNOCK_DOWN_MAP_VAR, (intptr_t)nullptr);
    return ApiStatus_DONE2;
}
