...
API_CALLABLE(N(BulletBillAI_Main)) {
    ...
    MobileAISettings* aiSettings = (MobileAISettings*)(intptr_t)evt_get_variable(script, *script->ptrReadPos);
...
API_CALLABLE(N(BillBlasterAI_Main)) {
    ...
    Enemy* enemy = script->owner1.enemy;
    Npc* npc = get_npc_unsafe(enemy->npcID);
    MobileAISettings* aiSettings = (MobileAISettings*)(intptr_t)evt_get_variable(script, *args++);
    s32 bulletNpcID;
    Enemy* bulletEnemy;
    f32 deltaY;
