...
#ifdef PORT
    f32 distance = dist2D(playerStatus->pos.x, playerStatus->pos.z,
        port_array_get(script->array, 2), port_array_get(script->array, 3));
#else
...
#ifdef PORT
    if (port_array_get(script->array, 4) <= distance) {
#else
...
API_CALLABLE(N(CheckShouldFlipFloor)) {
    ...
    f32 distance = dist2D(
        ...
        port_array_get(script->array, 2), port_array_get(script->array, 4));
#else
    f32 distance = dist2D(
        playerStatus->pos.x, playerStatus->pos.z,
