...
#ifdef PORT
                {
                    u32 bgRom = resolve_rom_offset(N(LetterDmaOffsets)[0]);
                    u32 bgPalRom = resolve_rom_offset((void*)N(LetterDmaOffsets)[1]);
                    if (bgRom != 0xFFFFFFFF) {
                        nuPiReadRom(bgRom, N(LetterBackgroundImg), sizeof(N(LetterBackgroundImg)));
                    } else {
                        memset(N(LetterBackgroundImg), 0, sizeof(N(LetterBackgroundImg)));
                    }
                    if (bgPalRom != 0xFFFFFFFF) {
                        nuPiReadRom(bgPalRom, N(LetterBackgroundPal), sizeof(N(LetterBackgroundPal)));
                    } else {
                        memset(N(LetterBackgroundPal), 0, sizeof(N(LetterBackgroundPal)));
                    }
                }
#else
...
#ifdef PORT
                {
                    u32 photoRom = resolve_rom_offset(N(LetterDmaOffsets)[(userIndex * 2) + 2]);
                    u32 photoPalRom = resolve_rom_offset((void*)N(LetterDmaOffsets)[(userIndex * 2) + 3]);
                    if (photoRom != 0xFFFFFFFF) {
                        nuPiReadRom(photoRom, N(LetterPhotoImg), sizeof(N(LetterPhotoImg)));
                    } else {
                        memset(N(LetterPhotoImg), 0, sizeof(N(LetterPhotoImg)));
                    }
                    if (photoPalRom != 0xFFFFFFFF) {
                        nuPiReadRom(photoPalRom, N(N(LetterPhotoPal)), sizeof(N(N(LetterPhotoPal))));
                    } else {
                        memset(N(N(LetterPhotoPal)), 0, sizeof(N(N(LetterPhotoPal))));
                    }
                }
#else
