...
API_CALLABLE(N(LoadHeartBlockMsgImg)) {
    ...
    N(HeartBlockMsgImg).width    = heart_block_img_width;
    N(HeartBlockMsgImg).height   = heart_block_img_height;
...
API_CALLABLE(N(func_80242F7C_8B2FEC)) {
    ...
    npc->verticalRenderOffset = sin_deg((*(f32*)&script->functionTemp[1])) * 1.5f;
    (*(f32*)&script->functionTemp[1]) = clamp_angle((*(f32*)&script->functionTemp[1]) + 18.0f);
    return ApiStatus_BLOCK;
}
