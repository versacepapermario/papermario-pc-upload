...
API_CALLABLE(N(CreateEndChapterData)) {
    ...
    if (isInitialCall) {
        ...
        evt_set_variable(script, MV_EndChapterDataPtr, (intptr_t) data);
...
API_CALLABLE(N(AddCardAngularVelocity)) {
    Bytecode* args = script->ptrReadPos;
    EndChapter* data = (EndChapter*)(intptr_t) evt_get_variable(script, MV_EndChapterDataPtr);
...
API_CALLABLE(N(AccelerateCardSpin)) {
    ...
    if (isInitialCall) {
        (*(f32*)&script->functionTemp[1]) = evt_get_float_variable(script, *args++);
        duration = script->functionTemp[2] = evt_get_variable(script, *args++);
        script->functionTemp[0] = 0;
        (*(f32*)&script->functionTemp[1]) = (*(f32*)&script->functionTemp[1]) / duration;
    }

    data = (EndChapter*)(intptr_t) evt_get_variable(script, MV_EndChapterDataPtr);
    data->angularVelocity += (*(f32*)&script->functionTemp[1]);
...
API_CALLABLE(N(EndOfChapterBounceIn)) {
    Bytecode* args = script->ptrReadPos;
    EndChapter* endChatper = (EndChapter*)(intptr_t) evt_get_variable(script, MV_EndChapterDataPtr);
    s32 posY;
...
API_CALLABLE(N(ShowMessagesInFrontOfCurtains)) {
    EndChapter* data = (EndChapter*)(intptr_t) evt_get_variable(nullptr, MV_EndChapterDataPtr);
    data->unk1E = true;
...
void func_80240DA4_9087D4(void) {
    ...
    if (D_802417CC_9091FC != 0 && D_802417D0_909200 != 0) {
        ...
        draw_box(DRAW_FLAG_NO_CLIP, WS_CUSTOM(&D_802417D8_909208), ulx, uly, 0, D_802417CC_9091FC, D_802417D0_909200, 180, 0, 1.0f, 1.0f,
