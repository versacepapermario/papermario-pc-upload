...
void N(appendGfx_sticker)(void* renderData) {
    ...
    StickerData* sticker = (StickerData*)(intptr_t) evt_get_variable(nullptr, MV_StickerData);
...
API_CALLABLE(N(CreateSticker)) {
...
#ifdef PORT
    dma_copy(
        icon_ROM_START + gItemIconRasterOffsets[itemID],
        icon_ROM_START + gItemIconRasterOffsets[itemID] + 0x200,
        iconImg);
    dma_copy(
    ...
    evt_set_variable(script, MV_StickerData, (intptr_t) sticker);
    evt_set_variable(script, MV_StickerImage, (intptr_t) iconImg);
    evt_set_variable(script, MV_StickerPalette, (intptr_t) iconPal);
...
API_CALLABLE(N(SetStickerPos)) {
    ...
    StickerData* sticker = (StickerData*)(intptr_t) evt_get_variable(script, MV_StickerData);
...
API_CALLABLE(N(JumpStickerTo)) {
    Bytecode* args = script->ptrReadPos;
    StickerData* data = (StickerData*)(intptr_t) evt_get_variable(script, MV_StickerData);
    f32 x, y, z, dist;
...
API_CALLABLE(N(DeleteSticker)) {
    ...
    evt_set_variable(script, MV_StickerData, (intptr_t)nullptr);
    evt_set_variable(script, MV_StickerImage, (intptr_t)nullptr);
    evt_set_variable(script, MV_StickerPalette, (intptr_t)nullptr);
