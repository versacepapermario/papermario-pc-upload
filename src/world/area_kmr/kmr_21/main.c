...
#ifdef PORT
    // Yay0-decompressed data is big-endian; read offset with byte-swap
    TitleImage = (IMG_PTR)((u8*)TitleData + read_be_u32(&TitleData->img_offset_title));
#else
    TitleImage = (IMG_PTR)(TitleData->img_offset_title + (intptr_t)TitleData);
#endif
