...
API_CALLABLE(IsEventForSourceRunning) {
    Bytecode* args = script->ptrReadPos;
    Bytecode outVar = *args++;
    Bytecode* sourceToFind = (Bytecode*)(intptr_t)evt_get_variable(script, *args++);
...
API_CALLABLE(SetPushBlock) {
    ...
    if (occupant == PUSH_GRID_BLOCK) {
        ...
        bind_trigger_1(&EVS_PushWall_PushBlock, TRIGGER_WALL_PUSH, blockEntityID + EVT_ENTITY_ID_BIT, (intptr_t)blockGrid, blockEntityID, 3);
