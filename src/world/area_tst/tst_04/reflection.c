...
API_CALLABLE(N(EnablePlayerReflection)) {
    port_array_set(script->array, 0, create_worker_frontUI(nullptr, &N(worker_render_player_reflection)));
...
API_CALLABLE(N(EnablePartnerReflection)) {
    ...
    port_array_set(script->array, 1, create_worker_scene(&N(worker_update_partner_reflection), nullptr));
