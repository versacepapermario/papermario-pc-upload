...
#ifdef PORT
    {
        u32 peachRom = resolve_rom_offset(charset_peach_letter_OFFSET);
        u32 peachPalRom = resolve_rom_offset(charset_peach_letter_pal_OFFSET);
        if (peachRom != 0xFFFFFFFF) {
            nuPiReadRom(peachRom, &N(PeachLetterImg), sizeof(N(PeachLetterImg)));
        } else {
            memset(&N(PeachLetterImg), 0, sizeof(N(PeachLetterImg)));
        }
        if (peachPalRom != 0xFFFFFFFF) {
            nuPiReadRom(peachPalRom, &N(PeachLetterPal), sizeof(N(PeachLetterPal)));
        } else {
            memset(&N(PeachLetterPal), 0, sizeof(N(PeachLetterPal)));
        }
    }
#else
