...
#define SEF_U16(ptr) bswap16(*(u16*)(ptr))
#else
#define SEF_U16(ptr) (*(u16*)(ptr))
#endif
...
#if 0 // Diagnostic: SFX enqueue logging (disabled — verified working)
    {
        static int sSfxEnqCount = 0;
        if (sSfxEnqCount < 10) {
            fprintf(stderr, "[sfx] au_sfx_enqueue_event: soundID=0x%08X vol=%d pitch=%d pan=%d\n",
                    soundID, volume, pitchShift, pan);
            sSfxEnqCount++;
        }
    }
#endif
            ...
            if (prereq != 0) {
                ...
                for (j = 0; j < ARRAY_COUNT(manager->players); j++) {
                    if (manager->players[j].curSoundID == prereq) {
                        ...
                        newRequest.pan = 0;
                        au_sfx_try_sound(manager, &newRequest, nullptr);
                        break;
                    }
                }
            } else {
        ...
        for (i = 0; i < pending; i++) {
            ...
            if (request->soundID & SOUND_ID_LOWER) {
                if (request->soundID & SOUND_ID_STOP) {
                    ...
                    au_sfx_try_sound(manager, request, nullptr);
                } else {
...
#if 0 // Diagnostic: SFX try_sound logging (disabled — verified working)
    {
        static int sSfxLogCount = 0;
        if (sSfxLogCount < 10) {
            fprintf(stderr, "[sfx] au_sfx_try_sound: soundID=0x%04X idx=%d\n", soundID, soundIndex);
            sSfxLogCount++;
        }
    }
#endif
                ...
                if (polyphonyMode == 0) {
                    ...
                    if (useSpecificPlayerMode == 0) {
                        if (!foundPlayer) {
                            ...
                            player = &manager->players[playerIndex];
                            if (player->sefDataReadPos == nullptr || priority >= player->priority) {
                                au_sfx_play_sound(manager, player, AU_FILE_RELATIVE(manager->sefData, SEF_U16(cmdList)), request, priority, 0);
                            }
                            return;
                        }
                    }
                    ...
                    cmdList = AU_FILE_RELATIVE(manager->sefData, SEF_U16(cmdList));
                        ...
                        for (playerIndex = 7; playerIndex >= 0; playerIndex--) {
                            if (SEF_U16(cmdList) != 0) {
                                ...
                                if (player->sefDataReadPos == BlankSEFData) {
                                    au_sfx_play_sound(manager, player, AU_FILE_RELATIVE(manager->sefData, SEF_U16(cmdList)), request, priority, exclusiveID);
                                    NEXT_POLY_TRACK;
                                }
                            } else {
                                NEXT_POLY_TRACK;
...
static void au_sfx_play_sound(SoundManager* manager, SoundPlayer* player, s8* readPos, SoundRequest* request, s32 priority, s32 exclusiveID) {
    if (manager->state == SND_MANAGER_STATE_ENABLED) {
        ...
        player->envelopCustomPressProfile = nullptr;
        ...
        player->alternativeDataPos = nullptr;
...
static void au_sfx_update_sequence(SoundManager* manager, SoundPlayer* player, AuVoice* voice, u8 voiceIdx) {
    ...
    if (player->alternativeDataPos != nullptr) {
    ...
    while (player->delay == 0) {
        ...
        if (opcode < 0x80) {
            ...
            if (voice->priority <= manager->priority) {
                ...
                if (player->envelopCustomPressProfile == nullptr) {
                    voice->envelope.cmdListPress = player->envelope.cmdListPress;
                    voice->envelope.cmdListRelease = player->envelope.cmdListRelease;
                } else {
...
static void au_SEFCmd_04_SetEnvelope(SoundManager* manager, SoundPlayer* player) {
    ...
    if (envPreset != nullptr && envPreset->count != 0) {
