...
void au_mseq_play_sequence(AmbienceManager* manager, MSEQHeader* mseqFile, s32 index) {
    ...
    readPos = AU_FILE_RELATIVE(mseqFile, mseqFile->dataStart);
...
void au_mseq_load_track_fade_info(AmbienceManager* manager, AmbiencePlayer* player) {
    ...
    if (count != 0 && mseqFile->trackSettingsOffset != 0) {
        trackData = AU_FILE_RELATIVE(mseqFile, mseqFile->trackSettingsOffset);
        while (count--) {
                        ...
                        if (i < player->lastVoiceIdx) {
                            ...
                            if (trackIdx != TRACK_ID_DRUM) {
...
#ifdef PORT
                                if (track->instrument == NULL) {
                                    fprintf(stderr, "[MSEQ] NULL instrument on track %d, skipping note\n", trackIdx);
                                    break;
                                }
#endif
                                ...
                                if (voice->instrument == NULL) {
                                    fprintf(stderr, "[MSEQ] NULL drum instrument bank=%d patch=%d, skipping\n", bankPatch >> 8, bankPatch & 0xFF);
                                    break;
                                }
#endif
