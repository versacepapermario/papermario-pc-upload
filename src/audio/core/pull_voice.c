...
Acmd* au_pull_voice(AuPVoice* pvoice, Acmd* cmdBufPos) {
    ...
    if (outCount != 0) {
        if (decoder->instrument->type == AL_ADPCM_WAVE) {
            ...
            if (looped) {
                ...
                decoder->memin = AU_PTR_CAST(decoder->instrument->wavData) + ADPCMFBYTES * ((s32)(decoder->loop.start >> LFSAMPLES) + 1);
                ...
                overFlow = decoder->memin + nbytes - (AU_PTR_CAST(decoder->instrument->wavData) + decoder->instrument->wavDataLength);
            ...
            if (decoder->loop.end < outCount + decoder->sample && decoder->loop.count != 0) {
                ...
                outp += dramAlign;
                decoder->memin = AU_PTR_CAST(decoder->instrument->wavData) + (decoder->loop.start << 1);
                decoder->sample = decoder->loop.start;
                op = outp;
                while (outCount > nSam){
                    op += nSam << 1;
    ...
    if (envMixer->motion == AL_STOPPED) {
        ...
        decoder->first = 1;
        decoder->sample = 0;
        decoder->memin = AU_PTR_CAST(decoder->instrument->wavData);
