        ...
        switch (*mesg_type) {
            case NU_SC_RETRACE_MSG:
                if (cmdList_len != 0 && nuAuTaskStop == NU_AU_TASK_RUN) {
                    ...
                    osSendMesgPtr(&nusched.audioRequestMQ, &nuAuTasks[cmdListIndex], OS_MESG_BLOCK);
                    nuAuCleanDMABuffers();
...
s32 nuAuDmaCallBack(s32 addr, s32 len, void *state, u8 useDma) {
...
#ifdef PORT
            return (intptr_t)(dmaPtr->ptr + addr - dmaPtr->startAddr);
#else
    ...
    if (dmaPtr == nullptr) {
#ifdef PORT
        return (intptr_t)nuAuDmaState.firstUsed->ptr;
#else
...
#ifdef PORT
    // On PC, wave data is in the ROM file on disk.
    // For DMA instruments, addr is a ROM offset (not a RAM pointer).
    // Use nuPiReadRom to read from the ROM file.
    nuPiReadRom((u32)addr, freeBuffer, 0x500);
    nuAuDmaNext++;
    return (intptr_t)((char*)freeBuffer + delta);
#else
