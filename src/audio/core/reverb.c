...
Acmd* au_pull_fx(AuFX* fx, Acmd* ptr, s16 wetDmem, s16 tempDmem) {
    ...
    /* PORT: Reverb re-enabled after fixing n_aPoleFilter DMEM address bug.
     * The pole filter's dmemAddr parameter is a packed buffer index (address >> 8)
     * that the N64 RSP reconstructs by multiplying by 2 * AUDIO_SAMPLES.
     * Our n_aPoleFilterImpl now does this reconstruction correctly. */
    ...
    for (delayIdx = 0; delayIdx < fx->delayCount; delayIdx++) {
        ...
        if (delay->activeResampler) {
            ...
            ramAlign = ((uintptr_t) rsOutPtr & 7) >> 1;
            rsOutPtr -= ramAlign;
            if (rsOutPtr < fx->base) {
                rsOutPtr += fx->length;
            }
