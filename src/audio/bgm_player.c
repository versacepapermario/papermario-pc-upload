...
void au_bgm_clear_custom_note_press(BGMPlayer* player, s32 index) {
    ...
    for (i = 0; i < 9; i++) {
#ifdef PORT
        // END marker: on big-endian N64, 0xFF00 stores as bytes [0xFF, 0x00].
        // Envelope reader checks u8* for 0xFF (ENV_CMD_END).
        // On little-endian, 0x00FF stores as [0xFF, 0x00] — same byte layout.
        *pos++ = 0x00FF;
#else
        *pos++ = 0xFF00;
#endif
...
#ifdef PORT
        cmd = read_be_u32(player->compReadPos);
        player->compReadPos++;
#else
...
void au_bgm_load_phrase(BGMPlayer* player, u32 cmd) {
    ...
    for (i = 0; i < ARRAY_COUNT(player->tracks); i++) {
        track = &player->tracks[i];
#ifdef PORT
        trackInfo = read_be_u32(trackList);
        trackList++;
#else
        trackInfo = *trackList++;
#endif
                ...
                while (track->delayTime == 0) {
                    ...
                    if (opcode < 0x80) {
                        ...
                        if (opcode < 0xD4) {
                            ...
                            bgm_args_done:
#ifdef PORT
                            // SeqArgs union overlays raw[4] bytes with multi-byte fields (u16/u32).
                            // Bytes are read from big-endian ROM data; swap multi-byte fields for little-endian PC.
                            {
                                u8 t;
                                switch (opcode) {
                                    case 0xE0: // MasterTempo: u16 at [0..1]
                                    case 0xEF: // TrackDetune: s16 at [0..1]
                                        t = player->seqCmdArgs.raw[0];
                                        player->seqCmdArgs.raw[0] = player->seqCmdArgs.raw[1];
                                        player->seqCmdArgs.raw[1] = t;
                                        break;
                                    case 0xE5: // MasterVolumeFade: u16 at [0..1]
                                    case 0xF6: // TrackVolumeFade: u16 at [0..1]
                                    case 0xFC: // Branch: u16 at [0..1]
                                    case 0xFE: // Detour: u16 at [0..1]
                                        t = player->seqCmdArgs.raw[0];
                                        player->seqCmdArgs.raw[0] = player->seqCmdArgs.raw[1];
                                        player->seqCmdArgs.raw[1] = t;
                                        break;
                                    ...
                                    case 0xFD: // EventTrigger: u32 at [0..3]
                                        ...
                                        break;
                                    default:
                                        break; // all-u8 fields: no swap needed
                                }
                            }
#endif
...
#ifdef PORT
                        // Custom envelopes are s16 arrays cast to u8* for envelope byte-stream reading.
                        // On big-endian N64, (hi << 8) | lo stores as memory bytes [hi, lo].
                        // On little-endian PC, swap so u8* reads see bytes in the same order.
                        player->customPressEnvelopes[i][writePos] = (arg2 << 8) | arg1;
#else
                        player->customPressEnvelopes[i][writePos] = (arg1 << 8) + arg2;
#endif
                    } else {
#ifdef PORT
                        player->customPressEnvelopes[i][writePos] = (arg2 << 8) | BgmCustomEnvLookup[arg1];
#else
