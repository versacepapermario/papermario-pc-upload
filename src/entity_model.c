                    ...
                    if (entityModel->nextFrameTime <= 0.0f) {
#ifdef PORT
                        {
                            ...
                            while (step_entity_model_commandlist(entityModel)) {
                                safety++;
                                if (safety > 10000) {
                                    fprintf(stderr, "[exec_entity_model_cmdlist] INFINITE LOOP idx=%d flags=0x%X\n",
                                            idx, entityModel->flags);
                                    fflush(stderr);
                                    break;
                                }
                            }
...
#ifdef PORT
    // EntityModelScript is intptr_t[] on 64-bit, so read as intptr_t* not u32*
    intptr_t* curPos = (intptr_t*)*entityModel->cmdListReadPos;
    switch ((u32)*curPos++) {
        case ENTITY_MODEL_SCRIPT_OP_End:
            free_entity_model_by_ref(entityModel);
            return 1;
        case ENTITY_MODEL_SCRIPT_OP_Draw:
            entityModel->nextFrameTime = (f32)(s32)*curPos++;
            entityModel->gfx.displayList = (Gfx*)*curPos++;
            entityModel->cmdListReadPos = (EntityModelScript*) curPos;
            break;
        case ENTITY_MODEL_SCRIPT_OP_Restart:
            entityModel->cmdListReadPos = entityModel->cmdListSavedPos;
            return 1;
        case ENTITY_MODEL_SCRIPT_OP_Loop:
            entityModel->cmdListReadPos = entityModel->cmdListSavedPos = (EntityModelScript*) curPos;
            return 1;
        case ENTITY_MODEL_SCRIPT_OP_SetRenderMode:
            entityModel->renderMode = (s32)*curPos++;
            entityModel->cmdListReadPos = (EntityModelScript*) curPos;
            return 1;
        case ENTITY_MODEL_SCRIPT_OP_SetFlags:
            entityModel->flags |= (u32)*curPos++;
            entityModel->cmdListReadPos = (EntityModelScript*) curPos;
            return 1;
        case ENTITY_MODEL_SCRIPT_OP_ClearFlags:
            entityModel->flags &= ~(u32)*curPos++;
            entityModel->cmdListReadPos = (EntityModelScript*) curPos;
            return 1;
        case ENTITY_MODEL_SCRIPT_OP_DrawImage:
            imageData = entityModel->gfx.imageData;
            entityModel->nextFrameTime = (s32)*curPos++;
            imageData->raster = (u8*)*curPos++;
            imageData->defaultPal = (PAL_PTR)*curPos++;
            imageData->width = (s32)*curPos++;
            imageData->height = (s32)*curPos++;
            entityModel->cmdListReadPos = (EntityModelScript*) curPos;
            break;
    }
#else
