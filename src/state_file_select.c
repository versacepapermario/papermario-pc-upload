...
// On PC, D_80200000 is already declared as Addr (unsigned char[]) in ld_addrs.h
#endif
...
void state_init_file_select(void) {
#ifdef PORT
    fprintf(stderr, "[state_init_file_select] start\n");
#endif
    D_800A0931 = 0;
    D_800A0932 = 0;
    disable_player_input();
    ...
    gOverrideFlags |= GLOBAL_OVERRIDES_WINDOWS_OVER_CURTAINS;
#ifdef PORT
    fprintf(stderr, "[state_init_file_select] done\n");
    fflush(stderr);
#endif
}
        ...
        case 2:
            if (D_800A0930 >= 0) {
                ...
                if (temp == 0) {
                    ...
                    battle_heap_create();
#ifdef PORT
                    // On PC, filemenu code is statically linked — skip ROM overlay load
#else
                    nuPiReadRomOverlay(&D_8007798C);
#endif
                    filemenu_init(0);
