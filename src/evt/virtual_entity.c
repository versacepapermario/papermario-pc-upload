...
#ifdef PORT
    // Skip off-screen rectangles from negative coordinates on widescreen PC
    if (posX < 0 || posY < 0 || endX > 340 || endY > 260) {
        return;
    }
#endif
...
API_CALLABLE(CreateVirtualEntityAt) {
    ...
    EntityModelScript* cmdList = (EntityModelScript*)(intptr_t) evt_get_variable(script, *args++);
