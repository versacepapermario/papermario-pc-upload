...
#ifdef PORT
    // load shading group data (big-endian s32[2] from ROM)
    dma_copy(sprite_shading_profiles_ROM_START + shadingGroupOffset,
             sprite_shading_profiles_ROM_START + shadingGroupOffset + 8, gSpriteShadingHeader);
    bswap32_array(gSpriteShadingHeader, 2);

    // load offset to shading data
    data = gSpriteShadingHeader[0];
    dma_copy(sprite_shading_profiles_ROM_START + gSpriteShadingHeader[1] + shadingProfileOffset,
             sprite_shading_profiles_ROM_START + gSpriteShadingHeader[1] + shadingProfileOffset + 4, gSpriteShadingHeader);
    bswap32_array(gSpriteShadingHeader, 1);

    // load shading data (byte array — no swap needed)
    dma_copy(sprite_shading_profiles_data_ROM_START + data + gSpriteShadingHeader[0],
             sprite_shading_profiles_data_ROM_START + data + gSpriteShadingHeader[0] + sizeof(gSpriteShadingData), &gSpriteShadingData);
#else
