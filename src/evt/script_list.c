...
// demo_api — actual sizes must match extern declarations in demo_api.c
// N64 relied on linker script placement; PC must allocate the correct sizes
BSS s32 gSpriteShadingHeader[2];   // demo_api.c: extern s32 gSpriteShadingHeader[2]
BSS u8 gSpriteShadingData[0x100];  // demo_api.c: extern u8 gSpriteShadingData[0x100]
...
#ifdef PORT
BSS intptr_t gWorldMapVars[MAX_MAPVARS];
BSS intptr_t gBattleMapVars[MAX_MAPVARS];
#else
...
#ifdef PORT
    if (i >= MAX_SCRIPTS) {
        fprintf(stderr, "[EVT] start_script_in_group: ALL %d slots full! source=%p\n", MAX_SCRIPTS, (void*)source);
        // Count unique source scripts and find the most common one
        void* topSrc = NULL;
        int topCount = 0;
        int activeCount = 0;
        for (int d = 0; d < MAX_SCRIPTS; d++) {
            Evt* s = (*gCurrentScriptListPtr)[d];
            if (s) {
                activeCount++;
                int cnt = 0;
                for (int e = 0; e < MAX_SCRIPTS; e++) {
                    Evt* s2 = (*gCurrentScriptListPtr)[e];
                    if (s2 && s2->ptrFirstLine == s->ptrFirstLine) cnt++;
                }
                if (cnt > topCount) { topCount = cnt; topSrc = (void*)s->ptrFirstLine; }
            }
        }
        fprintf(stderr, "[EVT] active=%d, most common script=%p (x%d)\n", activeCount, topSrc, topCount);
        // Print first 20 scripts
        for (int d = 0; d < 20; d++) {
            Evt* s = (*gCurrentScriptListPtr)[d];
            if (s) fprintf(stderr, "  [%03d] id=%-4d flags=0x%04x src=%p grp=0x%02x op=%d\n",
                d, s->id, s->stateFlags, (void*)s->ptrFirstLine, s->groupFlags, s->curOpcode);
        }
        fflush(stderr);
    }
#endif
...
#ifdef PORT
    // No-op on 64-bit: original code truncates Evt* pointers to s32,
    // crashing when dereferenced. The function has no side effects.
    return;
#else
    s32 temp;
    s32 i;
