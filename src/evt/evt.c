...
ApiStatus evt_handle_loop(Evt* script) {
    ...
    script->loopStartTable[loopDepth] = (intptr_t)args;
...
#ifdef PORT
/**
 * PORT: On 64-bit, Bytecode is intptr_t (8 bytes) but BufRead/UseBuf targets
 * are C structs with s32 fields (4 bytes). Reading via Bytecode* reads 8 bytes
 * and advances 8 bytes, causing wrong values and buffer overruns.
 * These helpers read/advance by s32 stride (4 bytes) instead.
 */
static inline s32 port_buf_read_next(Bytecode** bufPtr) {
    if (*bufPtr == NULL) {
        return 0;
    }
    s32 val = *(s32*)*bufPtr;
    *bufPtr = (Bytecode*)((s32*)*bufPtr + 1);
    return val;
}
...
ApiStatus evt_handle_get_1_word(Evt* script) {
    ...
    var = *args++;
#ifdef PORT
    evt_set_variable(script, var, port_buf_read_next(&script->buffer));
#else
...
ApiStatus evt_handle_get_2_word(Evt* script) {
    ...
    var2 = *args++;
#ifdef PORT
    evt_set_variable(script, var2, port_buf_read_next(&script->buffer));
#else
...
ApiStatus evt_handle_get_3_word(Evt* script) {
    ...
    var3 = *args++;
#ifdef PORT
    evt_set_variable(script, var3, port_buf_read_next(&script->buffer));
#else
...
ApiStatus evt_handle_get_4_word(Evt* script) {
    ...
    var4 = *args++;
#ifdef PORT
    evt_set_variable(script, var4, port_buf_read_next(&script->buffer));
#else
...
ApiStatus evt_handle_get_Nth_word(Evt* script) {
    ...
    var = *args++;
#ifdef PORT
    evt_set_variable(script, var, port_buf_peek(script->buffer, evt_get_variable(script, *args++)));
#else
...
ApiStatus evt_handle_get_1_float(Evt* script) {
    ...
    var = *args++;
#ifdef PORT
    evt_set_float_variable(script, var, evt_get_float_variable(script, port_buf_read_next(&script->buffer)));
#else
...
ApiStatus evt_handle_get_2_float(Evt* script) {
    ...
    var2 = *args++;
#ifdef PORT
    evt_set_float_variable(script, var2, evt_get_float_variable(script, port_buf_read_next(&script->buffer)));
#else
...
ApiStatus evt_handle_get_3_float(Evt* script) {
    ...
    var3 = *args++;
#ifdef PORT
    evt_set_float_variable(script, var3, evt_get_float_variable(script, port_buf_read_next(&script->buffer)));
#else
...
ApiStatus evt_handle_get_4_float(Evt* script) {
    ...
    var4 = *args++;
#ifdef PORT
    evt_set_float_variable(script, var4, evt_get_float_variable(script, port_buf_read_next(&script->buffer)));
#else
...
ApiStatus evt_handle_get_Nth_float(Evt* script) {
    ...
    var = *args++;
#ifdef PORT
    evt_set_float_variable(script, var, port_buf_peek(script->buffer, evt_get_variable(script, *args++)));
#else
...
#ifdef PORT
    // Point directly to the BSS s32 array. Multiple scripts calling
    // UseArray(same_address) must share the same backing storage.
    // ArrayVar reads/writes use s32 stride via port_array_get/set.
    script->array = (Bytecode*)(intptr_t)evt_get_variable(script, *script->ptrReadPos);
#else
    script->array = (s32*)(intptr_t)evt_get_variable(script, *script->ptrReadPos);
#endif
    return ApiStatus_DONE2;
}

ApiStatus evt_handle_set_flag_array(Evt* script) {
    script->flagArray = (s32*)(intptr_t)evt_get_variable(script, *script->ptrReadPos);
    return ApiStatus_DONE2;
}
...
ApiStatus evt_handle_allocate_array(Evt* script) {
    ...
    // Allocate s32 array — ArrayVar access uses s32 stride on PORT
    script->array = (Bytecode*)heap_malloc(size * sizeof(s32));
    evt_set_variable(script, var, (intptr_t)script->array);
    return ApiStatus_DONE2;
}
...
#ifdef PORT
        sEvtCallRing[sEvtCallRingIdx % 128] = (typeof(sEvtCallRing[0])){func, 'B'};
        sEvtCallRingIdx++;
#endif
...
#ifdef PORT
        sEvtCallRing[sEvtCallRingIdx % 128] = (typeof(sEvtCallRing[0])){(void*)script->callFunction, 'N'};
        sEvtCallRingIdx++;
#endif
...
ApiStatus evt_handle_jump(Evt* script) {
    ...
    script->ptrFirstLine = (Bytecode*)(intptr_t) evt_get_variable(script, *args++);
...
ApiStatus evt_handle_bind(Evt* script) {
    Bytecode* args = script->ptrReadPos;
    Trigger* trigger;
    Bytecode* triggerScript = (Bytecode*)(intptr_t)evt_get_variable(script, *args++);
    ...
    if (triggerOut != 0) {
        evt_set_variable(script, triggerOut, (intptr_t)trigger);
    }
...
API_CALLABLE(DeleteTrigger) {
    delete_trigger((Trigger*)(intptr_t)evt_get_variable(script, *script->ptrReadPos));
    return ApiStatus_DONE2;
}
...
ApiStatus evt_handle_bind_lock(Evt* script) {
    ...
    Bytecode triggerType = *args++;
    Bytecode colliderIDVar = *args++;
    s32* itemList = (s32*)(intptr_t)evt_get_variable(script, *args++);
    Bytecode triggerOut = *args++;
...
#ifdef PORT
        arrayVal = port_array_get(script->array, var);
#else
        arrayVal = script->array[var];
#endif

#ifdef PORT
        if (arrayVal <= EVT_LIMIT) {
#else
...
Bytecode evt_get_variable(Evt* script, Bytecode var) {
    ...
    if (var <= EVT_LIMIT) {
        ...
        var = EVT_INDEX_OF_ARRAY_VAR(var);
#ifdef PORT
        var = port_array_get(script->array, var);
#else
...
s32 evt_set_variable(Evt* script, Bytecode var, s32 value) {
    ...
    if (var <= EVT_LIMIT) {
        ...
        var = EVT_INDEX_OF_ARRAY_VAR(var);
#ifdef PORT
        oldValue = port_array_get(script->array, var);
        port_array_set(script->array, var, value);
#else
...
f32 evt_get_float_variable(Evt* script, Bytecode var) {
    ...
    if (var <= EVT_LIMIT) {
        ...
        var = EVT_INDEX_OF_ARRAY_VAR(var);
#ifdef PORT
        return evt_fixed_var_to_float(port_array_get(script->array, var));
#else
...
#ifdef PORT
        oldValue = port_array_get(script->array, var);
        port_array_set(script->array, var, evt_float_to_fixed_var(value));
#else
        oldValue = script->array[var];
