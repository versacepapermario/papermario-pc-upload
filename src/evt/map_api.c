...
API_CALLABLE(SetCustomGfx) {
    ...
    Gfx* pre = (Gfx*)(intptr_t) evt_get_variable(script, *args++);
    Gfx* post = (Gfx*)(intptr_t) evt_get_variable(script, *args++);
...
API_CALLABLE(ResetFromLava) {
    ...
    if (isInitialCall) {
        lavaReset = gLavaResetList = (LavaReset*)(intptr_t)evt_get_variable(script, *args++);
...
void goto_map(Evt* script, s32 mode) {
    ...
    if (mode == 2) {
        ...
        get_map_IDs_by_name((char*)(intptr_t)evt_get_variable(script, *args++), &areaID, &mapID);
    }
