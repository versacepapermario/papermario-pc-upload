...
#ifndef PORT
        // PORT: Partner battle code is statically linked, no DMA needed
...
#ifdef PORT
    // PORT: On 64-bit, the home union is pointer-sized (8 bytes). When .index is set
    // (small value 0-16), reading as uintptr_t gives the small value. When .vec is set
    // (pointer), reading as uintptr_t gives the full pointer. The N64 EVT_LIMIT check
    // fails because 64-bit pointers are positive, not negative like KSEG0 addresses.
    {
        uintptr_t home_val = *(uintptr_t*)&formation->home;
        if (home_val <= BTL_POS_CENTER) {
            x = StandardActorHomePositions[home_val].x;
            y = StandardActorHomePositions[home_val].y;
            z = StandardActorHomePositions[home_val].z;
        } else {
            Vec3i* vec = (Vec3i*)home_val;
            x = vec->x;
            y = vec->y;
            z = vec->z;
        }
...
#ifdef PORT
    // Skip off-screen rectangles from wrapped negative coordinates on widescreen PC
    if (left > 340 || right > 340 || top > 260 || bottom > 260) {
        return;
    }
#endif
