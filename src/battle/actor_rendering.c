...
void appendGfx_enemy_actor(void* data) {
    appendGfx_npc_actor(false, (s32)(intptr_t) data);
}

void appendGfx_partner_actor(void* data) {
    appendGfx_npc_actor(true, (s32)(intptr_t) data);
}
...
#ifdef PORT
                // Palette data is big-endian RGBA5551. Must byte-swap before
                // manipulating color bits, then swap back for Fast3D.
                u16 native = PAL_TO_NATIVE(*src);
                *dest = PAL_TO_BE(0xFFFE | (native & 1)); // white, preserving alpha bit
#else
...
void render_with_sleep_palettes(bool isNpcSprite, ActorPart* part, s32 yaw, Matrix4f mtx, bool skipAnimation) {
    ...
    for (i = 0; i < decorations->originalPalettesCount; i++) {
        ...
        if (palIn != nullptr) {
            for (j = 0; j < SPR_PAL_SIZE; j++) {
                u16 palColor = PAL_TO_NATIVE(*palIn);
                u8 r = UNPACK_PAL_R(palColor);
                u8 g = UNPACK_PAL_G(palColor);
                u8 b = UNPACK_PAL_B(palColor);
                u8 a = UNPACK_PAL_A(palColor);
                palIn++;
                ...
                *palOut++ = PAL_TO_BE((r << 11) | (g << 6) | (b << 1) | a);
            }
        }
    }
    ...
    if (decorations->palAnimState == 0) {
        if (!skipAnimation && decorations->nextPalTime != 0) {
            ...
            for (i = 0; i < decorations->spriteColorVariations; i++) {
                ...
                for (j = 0; j < SPR_PAL_SIZE; j++) {
                    u16 palC2 = PAL_TO_NATIVE(*color2);
                    u16 palC1 = PAL_TO_NATIVE(*color1);
                    u8 r2 = UNPACK_PAL_R(palC2);
                    u8 g2 = UNPACK_PAL_G(palC2);
                    u8 b2 = UNPACK_PAL_B(palC2);
                    u8 r1 = UNPACK_PAL_R(palC1);
                    u8 g1 = UNPACK_PAL_G(palC1);
                    u8 b1 = UNPACK_PAL_B(palC1);
                    u8 a1 = UNPACK_PAL_A(palC1);
                    color2++;
                    color1++;
                    ...
                    *palOut++ = PAL_TO_BE((r1 << 11) | (g1 << 6) | (b1 << 1) | a1);
                }
            }
            if (blendAlpha == 255) {
    ...
    switch (decorations->palAnimState) {
        ...
        case PAL_SWAP_A_TO_B:
            ...
            for (j = 0; j < SPR_PAL_SIZE; j++) {
                u16 palC2 = PAL_TO_NATIVE(*color2);
                u16 palC1 = PAL_TO_NATIVE(*color1);
                r2 = UNPACK_PAL_R(palC2);
                g2 = UNPACK_PAL_G(palC2);
                b2 = UNPACK_PAL_B(palC2);
                r1 = UNPACK_PAL_R(palC1);
                g1 = UNPACK_PAL_G(palC1);
                b1 = UNPACK_PAL_B(palC1);
                a1 = UNPACK_PAL_A(palC1);
                color2++;
                color1++;
                ...
                *outColor++ = PAL_TO_BE((r1 << 11) | (g1 << 6) | (b1 << 1) | a1);
            }
