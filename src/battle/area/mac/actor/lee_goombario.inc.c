...
API_CALLABLE(N(OpenTattleWindow)) {
    ...
    evt_set_variable(script, *args++, (intptr_t) fx_tattle_window(0, 106.0f, 144.0f, 0, 1.0f, 0));
    return ApiStatus_DONE2;
}

API_CALLABLE(N(HideTattleWindow)) {
    Bytecode* args = script->ptrReadPos;
    EffectInstance* tattleEffect = (EffectInstance*)(intptr_t) evt_get_variable(script, *args++);
