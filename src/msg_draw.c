...
void drawbox_message_delegate(intptr_t data, s32 baseX, s32 baseY, s32 width, s32 height, s32 opacity, s32 darkening) {
...
void draw_message_window(MessagePrintState* printer) {
    ...
    switch (printer->windowState) {
        ...
        case MSG_WINDOW_STATE_E:
            ...
            draw_box(DRAW_FLAG_ROTSCALE, WS_ID(WINDOW_STYLE_0), printer->windowBasePos.x, 56, 0, printer->windowSize.x, printer->windowSize.y, 255, 0,
...
void appendGfx_message(MessagePrintState* printer, s16 posX, s16 posY, u16 additionalOffsetX, u16 additionalOffsetY,
                ...
                switch (msg_drawState->printBuffer[msg_drawState->drawBufferPos + 1]) {
                    ...
                    case MSG_STYLE_B:
                        ...
                        if (printer->windowState == MSG_WINDOW_STATE_D || printer->windowState == MSG_WINDOW_STATE_E) {
#ifdef PORT
                            // PORT: ROTSCALE is stripped, so we render directly to screen.
                            // Use the correct centered position (box is already drawn by draw_message_window's draw_box).
                            printer->windowBasePos.x = 160 - printer->windowSize.x / 2;
                            printer->windowBasePos.y = 56;
#else
                            ...
                            printer->windowBasePos.x = 160 - printer->windowSize.x / 2;
                            printer->windowBasePos.y = 56;
                            draw_box(0, WS_ID(WINDOW_STYLE_0), printer->windowBasePos.x, 56, 0, printer->windowSize.x,
    ...
    if (printer->varImageScreenPos.x != 0) {
        ...
        if (varImgFinalAlpha > 0) {
            if (varImgHasBorder) {
                draw_box(0, WS_ID(WINDOW_STYLE_15), printer->varImageScreenPos.x - 7, printer->varImageScreenPos.y - 7, 0,
...
#ifdef PORT
    if (posX > 340 || lrX > 340 || posY > 260 || lrY > 260) {
        return;
    }
#endif
...
#if 0 // Diagnostic: speech bubble logging (disabled — verified working)
    {
        static int sBubbleLogCount = 0;
        if (sBubbleLogCount < 10) {
            extern unsigned char ui_msg_bubble_left_png[];
            extern unsigned char ui_msg_bubble_mid_png[];
            extern unsigned char ui_msg_palettes[16][32];
            u32 bck = 0;
            for (int i = 0; i < 64; i++) bck += ui_msg_bubble_left_png[i];
            u32 mck = 0;
            for (int i = 0; i < 64; i++) mck += ui_msg_bubble_mid_png[i];
            u32 pck = 0;
            for (int i = 0; i < 32; i++) pck += ui_msg_palettes[msg_drawState->framePalette][i];
            fprintf(stderr, "[speech_bubble] pos=%d,%d straight=%d curve=%d h=%d opacity=%d pal=%d bck64=%u mck64=%u pck=%u\n",
                posX, posY, straightWidth, curveWidth, height, opacity,
                msg_drawState->framePalette, bck, mck, pck);
            sBubbleLogCount++;
        }
    }
#endif
...
void msg_draw_frame(s32 posX, s32 posY, s32 sizeX, s32 sizeY, s32 style, s32 palette, s32 fading, s32 bgAlpha, s32 frameAlpha) {
    ...
    if (fading == 0 || bgAlpha != 0) {
        ...
        switch (style) {
            ...
            {
#ifdef PORT
                // Palette data is N64 big-endian RGBA5551; byte-swap for LE host
                u16 bgColor = __builtin_bswap16(((u16*)ui_msg_palettes)[4]);
#else
                u16 bgColor = ((u16*)ui_msg_palettes)[4];
#endif
                r = UNPACK_PAL_R(bgColor) * 8;
                g = UNPACK_PAL_G(bgColor) * 8;
                b = UNPACK_PAL_B(bgColor) * 8;
            }
                gDPPipeSync(gMainGfxPos++);
