...
#ifdef PORT
            fprintf(stderr, "[state_step_enter_world] load_map_by_IDs area=%d map=%d fromFileSelect=%d\n",
                    gGameStatusPtr->areaID, gGameStatusPtr->mapID, gLoadedFromFileSelect);
#endif
...
#ifdef PORT
            fprintf(stderr, "[state_step_enter_world] load_map_by_IDs done, mainScriptID=%d\n",
                    gGameStatusPtr->mainScriptID);
            {
                extern void worker_trigger_dump(void);
                worker_trigger_dump();
            }
#endif
    ...
    switch (gMapTransitionState) {
        ...
        case CHANGE_MAP_AWAIT_MAIN:
            ...
            if (gMapTransitionStateTime != 0) {
                ...
                // PORT: Force transition after timeout if main script is stuck
                // (e.g., waiting for dialogue input or missing NPC sprites)
                static s32 awaitMainTimeout = 0;
                awaitMainTimeout++;
                if (awaitMainTimeout > 90) { // ~3 seconds at 30fps
                    fprintf(stderr, "[state_step_change_map] PORT: forcing transition (main script %d still running after %d frames)\n",
                            gGameStatusPtr->mainScriptID, awaitMainTimeout);
                    gOverrideFlags &= ~GLOBAL_OVERRIDES_DISABLE_DRAW_FRAME;
                    gMapTransitionState++;
                    awaitMainTimeout = 0;
                }
