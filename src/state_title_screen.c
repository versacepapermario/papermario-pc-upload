...
void state_init_title_screen(void) {
    ...
    D_8014C248 = true;
    general_heap_create();
    gfx_texture_cache_clear(); // PORT: Invalidate stale GPU textures after heap reset
    clear_printers();
...
#ifdef PORT
    /*
     * On N64, TitleDataStruct offsets are relative to the buffer start.
     * The pattern "offset + (s32)base" works on 32-bit, but on 64-bit:
     *   1. The s32 fields are stored big-endian (from Yay0 decompression)
     *   2. (s32) truncates a 64-bit pointer
     * Fix: read offsets as big-endian, then use proper pointer arithmetic.
     */
    {
        u8* base = (u8*)TitleScreen_ImgList;
        u32 logoOff       = read_be_u32(base + 0);
        u32 copyrightOff  = read_be_u32(base + 4);
        u32 pressStartOff = read_be_u32(base + 8);
...
#ifdef PORT
                // PORT: Always play demo on idle — intro replay crashes in update_npcs
                // after demo has run (64-bit pointer issue in hos_05/hos_04 NPC scripts)
