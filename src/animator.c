...
Vtx* animator_copy_vertices_to_buffer(ModelAnimator* animator, AnimatorNode* node, Vec3s* buffer, s32 vtxCount,
    ...
    if (animator->baseAddr != nullptr) {
        i = ((intptr_t)buffer & 0xFFFFFF); // needed to match
        buffer = (Vec3s*)(i + (intptr_t)animator->baseAddr);
    }
#endif
...
s32 create_mesh_animator(s16* animPos, s16* animBuffer) {
    ...
    animator->timeScale = 1.0f;
#ifdef PORT
    // On PC, AnimScript arrays are statically compiled at valid PC addresses.
    // On N64, they were in DMA overlay data and needed segment offset extraction via & 0xFFFFFF.
    animator->animReadPos = animPos;
    animator->savedReadPos = animPos;
#else
...
s32 step_model_animator(ModelAnimator* animator) {
    ...
    switch (*args++) {
        ...
        case AS_SET_NODE_FLAGS:
            ...
            flags = *args++;
#ifdef PORT
            if (node == nullptr) { animator->animReadPos = args; return 1; }
#endif
            node->flags |= flags;
            animator->animReadPos = args;
            return 1;
...
void appendGfx_animator_node(ModelAnimator* animator, AnimatorNode* node, Matrix4f mtx) {
    ...
    if (node->displayList != nullptr) {
#ifdef PORT
        // Validate display list: first word's top byte must be a valid GBI opcode
        {
            uintptr_t firstW0 = node->displayList->words.w0;
            u8 opcode = (firstW0 >> 24) & 0xFF;
            // Valid F3DEX2 opcodes are in ranges 0x00-0x0F, 0xB0-0xFF
            if (opcode > 0x0F && opcode < 0xB0) {
                goto skip_dl;
            }
        }
#endif
        if (node->vertexStartOffset < 0) {
            ...
            do {
#ifdef PORT
                w0 = node->displayList[dlIdx].words.w0;
                w1 = node->displayList[dlIdx].words.w1;
                dlIdx++;
#else
                ...
                if (cmd == G_VTX) {
                    ...
                    if (node->fcData.vtxList == nullptr) {
                        newBuffer = &((Vtx*)(uintptr_t)w1)[node->vertexStartOffset + vtxIdx];
