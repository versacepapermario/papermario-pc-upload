...
void appendGfx_model(void* data) {
    ...
    switch (renderClass) {
        ...
        case RENDER_CLASS_2CYC_SHROUD:
...
#ifdef PORT
            // PORT: Fast3D doesn't support G_BL_CLR_BL (blend_color) as a blender source,
            // so PM_RM_SHROUD doesn't work. Emulate using standard fog blending:
            // - Use fog render modes (G_RM_FOG_SHADE_A + G_FOG geometry mode)
            // - Set fog_color = tint color
            // - Set fog_mul=0, fog_off=ShroudTintAmt for uniform tinting
            switch (renderMode) {
                case RENDER_MODE_SURFACE_OPA:
                    renderModeIdx = RENDER_MODE_IDX_1F;
                    break;
            ...
            gSPDisplayList((*gfxPos)++, ModelRenderModes[renderModeIdx]);
            gDPSetFogColor((*gfxPos)++, ShroudTintR, ShroudTintG, ShroudTintB, 255);
            gSPFogFactor((*gfxPos)++, 0, ShroudTintAmt);
#else
    ...
    if (!(flags & MODEL_FLAG_HAS_LOCAL_VERTEX_COPY)) {
        ...
        if (modelNode->displayData != NULL && modelNode->displayData->displayList != NULL) {
...
#ifdef PORT
/**
 * Byte-swap a TextureHeader read from big-endian ROM data.
 * Fixes u16 dimension fields and bitfield byte ordering (MIPS BE → x86 LE).
 */
static void texture_header_bswap(TextureHeader* header) {
    header->auxW  = ROM_BSWAP16(header->auxW);
    header->mainW = ROM_BSWAP16(header->mainW);
    header->auxH  = ROM_BSWAP16(header->auxH);
    header->mainH = ROM_BSWAP16(header->mainH);

    // Bitfield bytes: N64 big-endian MIPS packs first declared field in MSB,
    // x86 little-endian packs first declared field in LSB.
    u8* raw = (u8*)header;
    u8 b;

    // Byte 0x2A: auxCombineType:6 | auxCombineSubType:2
    b = raw[0x2A];
    raw[0x2A] = ((b & 0x03) << 6) | ((b >> 2) & 0x3F);

    // Byte 0x2B: auxFmt:4 | mainFmt:4
    b = raw[0x2B];
    raw[0x2B] = ((b & 0x0F) << 4) | ((b >> 4) & 0x0F);

    // Byte 0x2C: auxBitDepth:4 | mainBitDepth:4
    b = raw[0x2C];
    raw[0x2C] = ((b & 0x0F) << 4) | ((b >> 4) & 0x0F);

    // Byte 0x2D: auxWrapW:4 | mainWrapW:4
    b = raw[0x2D];
    raw[0x2D] = ((b & 0x0F) << 4) | ((b >> 4) & 0x0F);

    // Byte 0x2E: auxWrapH:4 | mainWrapH:4
    b = raw[0x2E];
    raw[0x2E] = ((b & 0x0F) << 4) | ((b >> 4) & 0x0F);
}
#endif
...
void load_texture_by_name(ModelNodeProperty* propertyName, s32 romOffset, s32 size) {
...
#ifdef PORT
        fprintf(stderr, "[model] texture NOT FOUND (overflow): '%s' (searched %d bytes from 0x%X)\n", textureName, size, startOffset);
#endif
        (*gCurrentModelTreeNodeInfo)[TreeIterPos].textureID = 0;
        return;
    }
    ...
    if (romOffset >= startOffset + size) {
        // loop ended without strcmp match, but didn't hit 0x40000 cap
        fprintf(stderr, "[model] texture NOT FOUND (exhausted): '%s' (size=%d, romOff=0x%X, start=0x%X)\n", textureName, size, romOffset, startOffset);
    }
#endif
...
void mdl_load_all_textures(ModelNode* rootModel, s32 romOffset, s32 size) {
    ...
    if (rootModel != nullptr && romOffset != 0 && size != 0) {
        ...
        // PORT: Clear Fast3D texture cache when reloading textures.
...
void mdl_calculate_model_sizes(void) {
    ...
    for (i = 0; i < ARRAY_COUNT(*gCurrentModels); i++) {
        ...
        if (model != nullptr) {
            ...
            bb->halfSizeX = (BB_MAX_X(bb) - BB_MIN_X(bb)) * 0.5;
            bb->halfSizeY = (BB_MAX_Y(bb) - BB_MIN_Y(bb)) * 0.5;
            bb->halfSizeZ = (BB_MAX_Z(bb) - BB_MIN_Z(bb)) * 0.5;
...
void mdl_create_model(ModelBlueprint* bp, s32 unused) {
        ...
        if (replaceWithFlame != 0) {
            ...
            if (prop != nullptr) {
                ...
                fx_flame(replaceWithFlame - 1,
                    (BB_MIN_X(bb) + BB_MAX_X(bb)) * 0.5f,
                    BB_MIN_Y(bb),
                    (BB_MIN_Z(bb) + BB_MAX_Z(bb)) * 0.5f,
                    1.0f,
                    &effect);
                return;
            }
    ...
    if (prop != nullptr) {
        ...
        x = (BB_MIN_X(bb) + BB_MAX_X(bb)) * 0.5f;
        y = (BB_MIN_Y(bb) + BB_MAX_Y(bb)) * 0.5f;
        z = (BB_MIN_Z(bb) + BB_MAX_Z(bb)) * 0.5f;
    } else {
        x = y = z = 0.0f;
    }
    ...
    x = BB_MAX_X(bb) - BB_MIN_X(bb);
    y = BB_MAX_Y(bb) - BB_MIN_Y(bb);
    z = BB_MAX_Z(bb) - BB_MIN_Z(bb);
    bb->halfSizeX = x * 0.5;
    bb->halfSizeY = y * 0.5;
    bb->halfSizeZ = z * 0.5;
    ...
    for (i = 0; i < ARRAY_COUNT(*gCurrentModels); i++) {
        ...
        if (model != nullptr && (model->flags != 0) && !(model->flags & MODEL_FLAG_INACTIVE)) {
            if (!(model->flags & MODEL_FLAG_MATRIX_DIRTY)) {
                ...
                mX = (BB_MIN_X(bb) + BB_MAX_X(bb)) * 0.5f;
                mY = (BB_MIN_Y(bb) + BB_MAX_Y(bb)) * 0.5f;
                mZ = (BB_MIN_Z(bb) + BB_MAX_Z(bb)) * 0.5f;
    ...
    for (i = 0; i < ARRAY_COUNT((*gCurrentTransformGroups)); i++) {
        ...
        if (mtg != nullptr && mtg->flags != 0 && !(mtg->flags & TRANSFORM_GROUP_FLAG_INACTIVE)) {
            if (!(mtg->flags & TRANSFORM_GROUP_FLAG_MATRIX_DIRTY)) {
                ...
                mtgX = (BB_MIN_X(bb) + BB_MAX_X(bb)) * 0.5f;
                mtgY = (BB_MIN_Y(bb) + BB_MAX_Y(bb)) * 0.5f;
                mtgZ = (BB_MIN_Z(bb) + BB_MAX_Z(bb)) * 0.5f;
...
#define TEST_POINT_VISIBILITY \
    ...
    yComp = outY * outW; \
    zComp = outZ * outW; \
    /* PORT: Widen X bounds for widescreen — the renderer scales X inward by */ \
    /* (4/3)/aspect, so NDC X range visible on screen is wider than [-1,+1]. */ \
    if (zComp > -1.0f && xComp >= -xBoundsNDC && xComp <= xBoundsNDC && yComp >= -1.0f && yComp <= 1.0f) { \
        break; \
    }
    ...
    for (i = 0; i < ARRAY_COUNT(*gCurrentModels); i++) {
        ...
        if (model->flags & MODEL_FLAG_DO_BOUNDS_CULLING) {
            ...
            // PORT: Skip culling if the camera is inside or very close to the bounding box.
            // The corner-based test fails when the model surrounds the camera (all corners
            // project outside the frustum even though the model fills the screen).
            {
                f32 camX = camera->lookAt_eye.x;
                f32 camY = camera->lookAt_eye.y;
                f32 camZ = camera->lookAt_eye.z;
                f32 margin = 50.0f; // extra margin to prevent popping
                if (camX >= centerX - bbx - margin && camX <= centerX + bbx + margin &&
                    camY >= centerY - bby - margin && camY <= centerY + bby + margin &&
                    camZ >= centerZ - bbz - margin && camZ <= centerZ + bbz + margin) {
                    goto skip_culling;
                }
            }

            while (true) {
                // Also test the center point — catches cases where corners all project
                // outside but the center is visible
                xComp = centerX;
                yComp = centerY;
                zComp = centerZ;
        ...
        case EXTRA_TILE_MIPMAPS:
            ...
            switch (mainBitDepth) {
                ...
                case G_IM_SIZ_32b:
                    ...
                    {
                        gDPLoadMultiTile((*gfxPos)++, rasterPtr, ((u32)(rasterPtr - raster)) >> 3, lod, mainFmt, G_IM_SIZ_32b,
                                         mainWidth / lodDivisor, mainHeight / lodDivisor,
                                         0, 0, mainWidth / lodDivisor - 1, mainHeight / lodDivisor - 1, 0,
                                         mainWrapW, mainWrapH, mainMasks - lod, mainMaskt - lod, lod, lod);
                    }
...
void mdl_get_vertex_count(Gfx* gfx, s32* numVertices, Vtx** baseVtx, s32* gfxCount, Vtx* baseAddr) {
    ...
    if (gfx == nullptr) {
        ...
        *baseVtx = nullptr;
        ...
        Gfx* baseGfx = gfx;
#ifdef PORT
        // PORT: After shape_swizzle, vertex arrays are in separate malloc'd blocks.
        // Address arithmetic (maxVtx - minVtx) doesn't work. Sum vertex counts instead.
        s32 totalVtxCount = 0;
        Vtx* firstVtx = NULL;
        ...
        *gfxCount = gfx - baseGfx;
        ...
        do {
            ...
            if (cmd == G_VTX) {
                vtxStartAddr = w1;
                if (baseAddr != nullptr) {
                    vtxStartAddr = (vtxStartAddr & 0xFFFF) + (intptr_t)baseAddr;
                }
                vtxCount = _SHIFTR(w0,12,8);
                if (minVtx == 0) {
                    minVtx = vtxStartAddr;
...
#ifdef PORT
/**
 * PORT: Non-contiguous vertex layout fix.
 * After shape_swizzle, each G_VTX group's vertices are in separate malloc'd blocks.
 * We copy the DL commands and remap each G_VTX to point into a single contiguous
 * vertex copy buffer, gathering vertices from each group sequentially.
 */
static void port_local_gfx_copy(Gfx* nodeDlist, s32 gfxCount, Gfx* gfxDst, Vtx* vtxDst) {
    s32 vtxOffset = 0;

    for (s32 i = 0; i < gfxCount; i++) {
        uintptr_t w0 = nodeDlist[i].words.w0;
        gfxDst[i].words.w0 = w0;

        if ((w0 >> 24) == G_VTX) {
            Vtx* srcVtx = (Vtx*)nodeDlist[i].words.w1;
            s32 numVerts = _SHIFTR(w0, 12, 8);
            // Copy this group's vertices into the contiguous buffer
            if (srcVtx != NULL) {
                memcpy(&vtxDst[vtxOffset], srcVtx, numVerts * sizeof(Vtx));
            }
            gfxDst[i].words.w1 = (uintptr_t)&vtxDst[vtxOffset];
            vtxOffset += numVerts;
        } else {
            gfxDst[i].words.w1 = nodeDlist[i].words.w1;
        }
    }
}
#else
...
void mdl_make_local_vertex_copy(s32 copyIndex, u16 modelID, s32 isMakingCopy) {
    ...
    if (isMakingCopy) {
#ifdef PORT
        // PORT: Create a contiguous source vertex copy for mdl_get_copied_vertices.
        // Consumer code indexes src[i] sequentially, so all vertices must be contiguous.
        Vtx* srcCopy = heap_malloc(numVertices * sizeof(Vtx));
        if (srcCopy != NULL) {
            // Gather all vertex groups into one contiguous buffer
            s32 vtxOff = 0;
            for (s32 j = 0; j < gfxCount; j++) {
                uintptr_t w0 = nodeDlist[j].words.w0;
                if ((w0 >> 24) == G_VTX) {
                    Vtx* sv = (Vtx*)nodeDlist[j].words.w1;
                    s32 nv = _SHIFTR(w0, 12, 8);
                    if (sv != NULL) {
                        memcpy(&srcCopy[vtxOff], sv, nv * sizeof(Vtx));
                    }
                    vtxOff += nv;
                }
            }
        }
        baseVtx = srcCopy; // Use contiguous copy as "source" for consumers
#endif
        for (i = 0; i < ARRAY_COUNT(copy->gfxCopy); i++) {
...
#ifdef PORT
            if (copy->gfxCopy[i] == NULL || copy->vtxCopy[i] == NULL) {
                fprintf(stderr, "[mdl_make_local_vertex_copy] heap_malloc failed (modelID=%d numVtx=%d gfxCount=%d)\n",
                        modelID, numVertices, gfxCount);
                for (; i < ARRAY_COUNT(copy->gfxCopy); i++) {
            ...
            port_local_gfx_copy(nodeDlist, gfxCount, copy->gfxCopy[i], copy->vtxCopy[i]);
#else
...
void* mdl_get_next_texture_address(s32 size) {
    uintptr_t offset = (uintptr_t)(TextureHeapPos - TextureHeapBase) + 0x3F;
