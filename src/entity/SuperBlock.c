...
#include "entity.h"
#ifdef PORT
// Pre-converted SuperBlockContent CI4 texture → RGBA16 (preserves color + alpha)
// 4 palette variants for the color cycling animation, each 32×32×2 = 2048 bytes
static u8 sSuperBlockContentRGBA16[4][32 * 32 * 2];
static s32 sSuperBlockContentConverted = FALSE;
#endif
...
// PORT helper: inline IA8 textured quad (used for all 3 star passes)
#ifdef PORT
static void superblock_draw_rgba16_quad(Gfx** gfxPos, s32 palIdx) {
    extern Vtx Entity_SuperBlockContent_vtx[];
    gDPSetTextureLUT((*gfxPos)++, G_TT_NONE);
    gSPTexture((*gfxPos)++, 0xFFFF, 0xFFFF, 0, G_TX_RENDERTILE, G_ON);
    gDPLoadTextureBlock((*gfxPos)++, sSuperBlockContentRGBA16[palIdx], G_IM_FMT_RGBA, G_IM_SIZ_16b, 32, 32, 0,
    ...
    gSPClearGeometryMode((*gfxPos)++, G_CULL_BACK | G_LIGHTING | G_SHADING_SMOOTH);
    gSPVertex((*gfxPos)++, Entity_SuperBlockContent_vtx, 4, 0);
...
void entity_SuperBlockContent_setupGfx(s32 entityIndex) {
...
#ifdef PORT
    {
        ...
        superblock_draw_rgba16_quad(&gfxPos, Entity_SuperBlock_PalData[data->paletteArrOffset + 1]);
    }
#else
    ...
    // Pass 2: trailing star (alpha=120)
    alpha = 120;
...
#ifdef PORT
    // CI4 rendering broken on PORT — pre-convert to RGBA16 (preserves color + alpha).
    // All 4 palette variants converted for the color cycling animation.
    if (!sSuperBlockContentConverted) {
        extern u8 D_0A000000_E4A6A0[];
        u8* ci4Data = D_0A000000_E4A6A0;
        u8* palettes[4] = { D_0A000200_E4A8A0, D_0A000220_E4A8C0, D_0A000240_E4A8E0, D_0A000260_E4A900 };
        for (int p = 0; p < 4; p++) {
            u8* palData = palettes[p];
            u8* out = sSuperBlockContentRGBA16[p];
            for (int i = 0; i < 32 * 32 / 2; i++) {
                u8 byte = ci4Data[i];
                int idx0 = (byte >> 4) & 0xF;
                int idx1 = byte & 0xF;
                out[i * 4 + 0] = palData[idx0 * 2 + 0];
                out[i * 4 + 1] = palData[idx0 * 2 + 1];
                out[i * 4 + 2] = palData[idx1 * 2 + 0];
                out[i * 4 + 3] = palData[idx1 * 2 + 1];
            }
        }
        sSuperBlockContentConverted = TRUE;
    }
#endif
}
