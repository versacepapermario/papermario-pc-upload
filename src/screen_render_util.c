...
#ifdef PORT
    // PORT: N64 reads the previous frame's RDRAM framebuffer as a texture.
    // On PC, GPU framebuffer readback is not available outside display list processing.
    // Draw a semi-transparent dark rectangle as a fallback.
    if (alpha > 0.0f) {
        ...
        gDPSetPrimColor(gMainGfxPos++, 0, 0, 0, 0, 0, (s32)alpha);
        gDPFillRectangle(gMainGfxPos++, x1, y1, x2, y2);
...
void appendGfx_screen_transition_stencil(s32 arg0, s32 arg1, f32 progress, s32 primR, s32 primG, s32 primB, s32 primA, s32 camID) {
    ...
    texScale = (255.0f - progress) * 10.5f / 255.0f + 0.09;
#ifdef PORT
    if (progress == 255.0f || texScale < 1.0f) {
    ...
    // s32 first ensures correct two's complement representation.
