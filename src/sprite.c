...
s32 spr_update_player_sprite(s32 spriteInstanceID, s32 animID, f32 timeScale) {
    ...
    if (PlayerSprites[spriteIdx] == nullptr) {
        return 0;
    }

#ifdef PORT
    {
        SpriteAnimData* sd = PlayerSprites[spriteIdx];
        rasterList = sd->rastersOffset;
        // For has_back sprites (e.g. Peach1_Back, Mario1_Back), the back sprite has 0 animations.
        // All animation data lives in the front sprite (spriteIdx - 1).
        if (sd->animListStart[0] == PTR_LIST_END && spriteIdx > 0 && PlayerSprites[spriteIdx - 1] != nullptr) {
...
s32 spr_draw_player_sprite(s32 spriteInstanceID, s32 yaw, s32 alphaIn, PAL_PTR* paletteList, Matrix4f mtx) {
...
#ifdef PORT
        static int sLogCount1 = 0;
        if (sLogCount1++ < 3) fprintf(stderr, "[spr_draw_player] SKIP: animID=ANIM_LIST_END inst=%d\n", instanceIdx);
#endif
        return false;
    }
    ...
    if (PlayerSprites[spriteIdx] == nullptr) {
        ...
        if (sLogCount2++ < 3) fprintf(stderr, "[spr_draw_player] SKIP: PlayerSprites[%d]=NULL animID=0x%X\n", spriteIdx, animID);
#endif
        return false;
    }
    ...
    {
        SpriteAnimData* sd = PlayerSprites[spriteIdx];
        rasters = sd->rastersOffset;
        palettes = sd->palettesOffset;
        // For has_back sprites, redirect animation access to front sprite (which has animations)
        SpriteAnimData* animSd = sd;
        if (sd->animListStart[0] == PTR_LIST_END && spriteIdx > 0 && PlayerSprites[spriteIdx - 1] != nullptr) {
            animSd = PlayerSprites[spriteIdx - 1];
        }
        animComponents = animSd->animListStart[SPR_UNPACK_ANIM(animID)];
    }
#else
    spriteData = (u32*)PlayerSprites[spriteIdx];
...
#ifdef PORT
                if (PlayerSprites[spriteIdBackFacing] != nullptr) {
...
s32 spr_load_npc_sprite(s32 animID, u32* extraAnimList) {
...
#ifdef PORT
    if (spriteIndex == 0 || spriteIndex > MAX_SPRITE_ID) {
        fprintf(stderr, "[spr_load_npc_sprite] ERROR: invalid spriteIndex=%d (animID=0x%X)\n",
                spriteIndex, animID);
        return -1;
    }
#endif
    ...
    if (NpcSpriteData[spriteIndex] != nullptr) {
        ...
        if (header == NULL) {
#ifdef PORT
            fprintf(stderr, "[spr_load_npc_sprite] ERROR: spr_load_sprite returned NULL for spriteIndex=%d\n", spriteIndex);
#endif
            NpcSpriteInstanceCount[spriteIndex] = 0;
            return -1;
        }
        SpriteInstances[listIndex].spriteData = header;
...
#ifdef PORT
    // On 64-bit, use proper struct member access instead of raw u32* arithmetic
    {
        SpriteAnimData* sd = SpriteInstances[i].spriteData;
        if (sd == NULL) {
            fprintf(stderr, "[spr_update_sprite] ERROR: NULL spriteData (instanceID=%d, i=%d, animID=0x%X)\n",
                    spriteInstanceID, i, animID);
            return 0;
        }
        rasterList = sd->rastersOffset;
        animList = sd->animListStart[animIndex];
...
#ifdef PORT
    // On 64-bit, use proper struct member access
    {
        SpriteAnimData* sd = SpriteInstances[i].spriteData;
        rasters = sd->rastersOffset;
        palettes = sd->palettesOffset;
        animComponents = sd->animListStart[SPR_UNPACK_ANIM(animID)];
    }
#else
    spriteData = (u32*)SpriteInstances[i].spriteData;
