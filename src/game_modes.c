...
void set_game_mode(s16 mode) {
    ...
    static const char* modeNames[] = {
        "STARTUP", "LOGOS", "TITLE_SCREEN", "ENTER_DEMO_WORLD",
        "WORLD", "CHANGE_MAP", "GAME_OVER", "ENTER_WORLD",
        "BATTLE", "END_BATTLE", "PAUSE", "UNPAUSE",
        "LANGUAGE_SELECT", "END_LANGUAGE_SELECT", "FILE_SELECT", "END_FILE_SELECT",
        "INTRO", "DEMO"
    };
    const char* name = (mode >= 0 && mode <= 17) ? modeNames[mode] : "UNKNOWN";
    fprintf(stderr, "[set_game_mode] %d -> %d (%s)\n", CurGameMode, mode, name);
    fflush(stderr);
#endif
    CurGameMode = mode;
