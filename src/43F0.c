...
u32 dma_copy(Addr romStart, Addr romEnd, void* vramDest) {
#ifdef PORT
    // On PC, resolve stub symbol addresses to actual ROM offsets
    u32 realStart = resolve_rom_offset(romStart);
    u32 realEnd = resolve_rom_offset(romEnd);
    u32 length;
    s32 i;

    if (realStart == 0xFFFFFFFF || realEnd == 0xFFFFFFFF) {
        fprintf(stderr, "[dma_copy] PORT: unknown ROM segment (start=%p end=%p) caller=%p\n",
                (void*)romStart, (void*)romEnd, __builtin_return_address(0));
        fflush(stderr);
        return 0;
    }
    ...
    for (i = 0; i + ROM_CHUNK_SIZE < length; i += ROM_CHUNK_SIZE) {
        nuPiReadRom(realStart + i, vramDest + i, ROM_CHUNK_SIZE);
    }

    if (i != length) {
        nuPiReadRom(realStart + i, vramDest + i, length - i);
    }
...
#ifdef PORT
    // Skip negative/wrapped coordinates that produce off-screen rectangles on widescreen PC
    if (left < 0 || right > 340 || top < 0 || bottom > 260) {
        return;
    }
#endif
