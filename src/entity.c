...
#if defined(SHIFT) || defined(PORT)
extern Addr WorldEntityHeapBottom;
extern Addr WorldEntityHeapBase;
#define WORLD_ENTITY_HEAP_BOTTOM (intptr_t) WorldEntityHeapBottom
#define WORLD_ENTITY_HEAP_BASE (intptr_t) WorldEntityHeapBase
...
#ifdef PORT
intptr_t CreateEntityVarArgBuffer[4];
#else
...
intptr_t gEntityHeapBase;
s32 gLastCreatedEntityIndex;

intptr_t gEntityHeapBottom;
                            ...
                            if (entity->scriptDelay == 0) {
#ifdef PORT
                                {
                                    ...
                                    while (step_entity_commandlist(entity)) {
                                        safety++;
                                        if (safety > 10000) {
                                            fprintf(stderr, "[update_entities] INFINITE LOOP in step_entity_commandlist entity[%d] flags=0x%08X scriptReadPos=%p\n",
                                                    i, entity->flags, (void*)entity->scriptReadPos);
                                            fflush(stderr);
                                            break;
                                        }
                                    }
            ...
            if (!(entity->flags & ENTITY_FLAG_HIDDEN)) {
                if (entity->flags & ENTITY_FLAG_HAS_ANIMATED_MODEL) {
                    if (D_8014AFB0 == 255) {
                        if (entity->renderSetupFunc != nullptr) {
                            set_animator_render_callback(
                                ...
                                (void*)(uintptr_t) entity->listIndex,
                    ...
                    if (D_8014AFB0 == 255) {
                        ...
                        bind_entity_model_setupGfx(entity->virtualModelIndex, (void*)(uintptr_t)entity->listIndex, func_8010FE44);
                    }
    ...
    for (i = 0; i < MAX_SHADOWS; i++) {
        ...
        if (shadow != nullptr) {
            if (shadow->flags & ENTITY_FLAG_HIDDEN) {
                ...
                bind_entity_model_setupGfx(shadow->entityModelID, (void*)(uintptr_t)shadow->alpha, entity_model_set_shadow_color);
...
void clear_entity_data(bool arg0) {
    ...
    if (gGameStatusPtr->context == CONTEXT_WORLD) {
        ...
        gEntityHeapBottom = (intptr_t) BattleEntityHeapBottom;
...
#ifdef PORT
            // PORT: skip animated entity reload (N64 struct layout mismatch)
            continue;
#else
...
void load_split_entity_data(Entity* entity, EntityBlueprint* entityData, s32 listIndex) {
    ...
    if (entityData->flags & ENTITY_FLAG_HAS_ANIMATED_MODEL) {
        ...
        if (gfxSize == 0) {
            fprintf(stderr, "[entity] PORT: gfx DMA size=0, start=%p end=%p\n", dmaList[0].start, dmaList[0].end);
            entity->flags |= ENTITY_FLAG_SKIP_UPDATE | ENTITY_FLAG_HIDDEN;
            return;
        }
        u32 animRomStart = resolve_rom_offset(dmaList[1].start);
        u32 animRomEnd   = resolve_rom_offset(dmaList[1].end);
        u32 animSize = (animRomStart != 0xFFFFFFFF && animRomEnd != 0xFFFFFFFF) ? (animRomEnd - animRomStart) : 0;
        if (animSize == 0) {
            fprintf(stderr, "[entity] PORT: anim DMA size=0, start=%p end=%p\n", dmaList[1].start, dmaList[1].end);
            entity->flags |= ENTITY_FLAG_SKIP_UPDATE | ENTITY_FLAG_HIDDEN;
            return;
        }

        // Allocate contiguously: [gfx][anim] — matches N64 layout where segment 0x0A
        // covers both. DL offsets > gfxSize point into the anim portion.
        u8* combinedBuf = malloc(gfxSize + animSize);
        dma_copy(dmaList[0].start, dmaList[0].end, combinedBuf);
        dma_copy(dmaList[1].start, dmaList[1].end, combinedBuf + gfxSize);
        entity->gfxBaseAddr = combinedBuf;

        u8* animBuf = combinedBuf + gfxSize;

        // Convert N64 data to PC format (nodes, DLs, vertices, byte-swap script)
        // Pass combined buffer size as gfxSize so DL offsets into anim region work
        s32 nodeCount = entity_convert_anim_data_port(
            entityData, animBuf, animSize, combinedBuf, gfxSize + animSize, &animationNodes);

        if (nodeCount > 0 && animationNodes != NULL) {
            animationScript = entityData->renderCommandList;
            entity->virtualModelIndex = create_mesh_animator(animationScript, (s16*)animBuf);
            load_mesh_animator_tree(entity->virtualModelIndex, animationNodes);
            update_model_animator(entity->virtualModelIndex);
            entity->flags |= ENTITY_FLAG_HAS_ANIMATED_MODEL;
        } else {
            fprintf(stderr, "[entity] PORT: animated entity conversion failed type=%d\n",
...
void load_split_entity_data(Entity* entity, EntityBlueprint* entityData, s32 listIndex) {
    ...
    animationScript = entityData->renderCommandList;
    animationNodes = (StaticAnimatorNode**)((intptr_t)animBaseAddr + (intptr_t)entityData->modelAnimationNodes);
    if (swizzlePointers) {
...
#ifdef PORT
        // PORT: Read as intptr_t to preserve 64-bit pointers (e.g., Tweester paths, BlueWarpPipe scripts)
...
API_CALLABLE(MakeEntity) {
    ...
    s32 entityIndex;
    intptr_t endOfArgs;
    intptr_t* varArgBufPos;

    if (isInitialCall != true) {
        return ApiStatus_DONE2;
    }

    entityData = (EntityBlueprint*)(intptr_t)evt_get_variable(script, *args++);
    ...
    *varArgBufPos = 0;
    ...
    do {
        // Read full pointer-width value from EVT bytecode (Ref() pointers need 64 bits)
        Bytecode rawArg = *args++;
        nextArg = (intptr_t)evt_get_variable(script, rawArg);
        // If the raw bytecode value looks like a direct pointer (not a variable reference),
        // use the full bytecode value to preserve 64-bit pointers
        if (rawArg > EVT_LIMIT && rawArg != (Bytecode)(s32)MAKE_ENTITY_END) {
            nextArg = rawArg;
        }
...
API_CALLABLE(AssignScript) {
    ...
    if (isInitialCall == true) {
        EvtScript* toBind = (EvtScript*)(intptr_t)evt_get_variable(script, *args++);
