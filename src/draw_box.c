...
s32 draw_box(s32 flags, WindowStyle windowStyle, s32 posX, s32 posY, s32 posZ, s32 width, s32 height, u8 opacity,
              ...
              void (*fpDrawContents)(intptr_t, s32, s32, s32, s32, s32, s32), void* drawContentsArg0, Matrix4f rotScaleMtx,
              s32 translateX, s32 translateY, Matrix4f outMtx)
{
    // (ROTSCALE render-to-texture handled below in PORT path)
...
#if 0 // Diagnostic: draw_box style/texture logging (disabled — verified working)
#ifdef PORT
        {
            static int sDrawBoxLogCount = 0;
            if (sDrawBoxLogCount < 30) {
                s32 styleID = (defaultStyle != nullptr) ? windowStyle.defaultStyleID : -1;
                u32 bgCk = 0, crCk = 0;
                if (bgImage) {
                    u32 bgSz = background->size;
                    for (u32 bi = 0; bi < bgSz && bi < 128; bi++) bgCk += bgImage[bi];
                }
                if (cornersImage) {
                    u32 crSz = 256;
                    for (u32 ci = 0; ci < crSz; ci++) crCk += cornersImage[ci];
                }
                fprintf(stderr, "[draw_box] style=%d pos=(%d,%d) size=(%d,%d) opacity=%d bgFmt=%d/%d bg=%dx%d bgImg=%p bgCk=%u corners=%d/%d cornersImg=%p crCk=%u combIdx=%d primA=%d flags=0x%X\n",
                    styleID, posX, posY, width, height, opacity,
                    bgFmt, background->bitDepth, bgWidth, bgHeight, (void*)bgImage, bgCk,
                    cornersFmt, cornersBitDepth, (void*)cornersImage, crCk,
                    defaultStyle ? defaultStyle->combineModeIndex : -1,
                    primA, flags);
                sDrawBoxLogCount++;
            }
        }
...
#ifdef PORT
        // PORT: Fast3D doesn't support G_BL_CLR_BL (blend_color) in the blender,
        // so PM_RM_SHROUD darkening has no effect. Apply darkening by pre-scaling
        // the primitive and environment colors instead.
        if (darkening > 0) {
            u16 darkenFactor = 255 - darkening;
            primR = primR * darkenFactor / 255;
            primG = primG * darkenFactor / 255;
            primB = primB * darkenFactor / 255;
            envR = envR * darkenFactor / 255;
            envG = envG * darkenFactor / 255;
            envB = envB * darkenFactor / 255;
        }
#endif
...
#ifdef PORT
            // PORT: N64 ROTSCALE renders contents to an offscreen CPU buffer via
            // gDPSetColorImage, then texture-maps it onto a rotated 3D quad.
            // Fast3D can't redirect rendering to arbitrary CPU addresses.
            // Skip content drawing during rotation frames (the box background still
            // rotates via the 3D quad path above). Content appears when rotation ends.
            if (quads == nullptr) {
                fpDrawContents((intptr_t)drawContentsArg0, posX, posY, width, height, opacity, darkening);
            }
#else
            if (quads != nullptr) {
                ...
                if(mdl_address != 0) {
                    ...
                    fpDrawContents((intptr_t)drawContentsArg0, 0, 0, width, height, opacity, darkening);
