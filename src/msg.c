...
#endif
#ifdef PORT
    // PORT: Resolve the OFFSET symbol to its absolute ROM address and read from ROM file.
    // On N64, charset_*_OFFSET symbols held numeric offsets within the charset segment.
    // On PC, they're stub arrays — resolve_rom_offset() maps them to actual ROM addresses.
    {
        u32 rom_addr = resolve_rom_offset(offset);
        if (rom_addr != 0xFFFFFFFF && dest != NULL && size > 0) {
            nuPiReadRom(rom_addr, dest, size);
        } else {
            fprintf(stderr, "[load_font_data] WARNING: could not resolve offset %p (size=0x%x)\n",
                    (void*)offset, size);
            fflush(stderr);
            if (dest != NULL && size > 0) {
                memset(dest, 0, size);
...
#ifdef PORT
                        {
                            u32 postcardRom = resolve_rom_offset(charset_postcard_OFFSET);
                            u32 postcardSize = (charset_postcard_png_width * charset_postcard_png_height) / 2;
                            if (postcardRom != 0xFFFFFFFF) {
                                nuPiReadRom(postcardRom, printer->letterBackgroundImg, postcardSize);
                            } else {
                                memset(printer->letterBackgroundImg, 0, postcardSize);
                            }
                        }
#else
...
#ifdef PORT
                        {
                            u32 postcardPalRom = resolve_rom_offset(charset_postcard_pal_OFFSET);
                            if (postcardPalRom != 0xFFFFFFFF) {
                                nuPiReadRom(postcardPalRom, printer->letterBackgroundPal, 0x20);
                            } else {
                                memset(printer->letterBackgroundPal, 0, 0x20);
                            }
                        }
#else
...
#ifdef PORT
                        {
                            u32 letterRom = resolve_rom_offset(MsgLetterRasterOffsets[arg]);
                            u32 letterSize = charset_letter_content_1_png_width * charset_letter_content_1_png_height;
                            if (letterRom != 0xFFFFFFFF) {
                                nuPiReadRom(letterRom, printer->letterContentImg, letterSize);
                            } else {
                                memset(printer->letterContentImg, 0, letterSize);
                            }
                        }
#else
...
#ifdef PORT
                        {
                            u32 letterPalRom = resolve_rom_offset(MsgLetterPaletteOffsets[arg]);
                            if (letterPalRom != 0xFFFFFFFF) {
                                nuPiReadRom(letterPalRom, printer->letterContentPal, 0x200);
                            } else {
                                memset(printer->letterContentPal, 0, 0x200);
                            }
                        }
#else
...
void dma_load_msg(u32 msgID, void* dest) {
    ...
    // US ROM has 47 valid sections (0x00-0x2E). Section 0x2F exists in JP only
    // (partner move rank names); US aliases these to section 0x2B via #defines in 5B320.c.
    // Sections >= 0x30 read past the section table, producing garbage offsets that cause
    // OOB ROM reads or oversized DMA that corrupts the heap.
    u32 section = msgID >> 16;
    if (section > 0x2F) {
        ((u8*)dest)[0] = 0xFD; // MSG_CHAR_READ_END
        return;
    }

    u32 sectionIdx = section * 4;
    u32 sectionOffset;
    u32 msgOffsets[2]; // start, end offsets
    ...
    // (e.g. section 0x2F is JP-only, US ROM has offset 0 for it)
    if (sectionOffset == 0) {
        fprintf(stderr, "[MSG] dma_load_msg: section 0x%X has zero offset (msgID=0x%X) — not in this ROM\n", section, msgID);
        ((u8*)dest)[0] = 0xFD; // MSG_CHAR_READ_END
        return;
    }
    ...
    // 3. Load the message data (cap to 0x3FF to prevent heap corruption)
    u32 msgStart = MSG_ROM_START + msgOffsets[0];
    u32 msgEnd = MSG_ROM_START + msgOffsets[1];
    u32 msgSize = msgEnd - msgStart;
    ...
    if (msgEnd > msgStart && msgSize < 0x400) {
        ...
        nuPiReadRom(msgStart, dest, 0x3FF);
        ((u8*)dest)[0x3FE] = 0xFD;
    } else {
        // Invalid offsets — write terminator
        fprintf(stderr, "[MSG] dma_load_msg: INVALID offsets for msgID=0x%X: start=0x%X end=0x%X (section=0x%X sectionOff=0x%X)\n",
                msgID, msgStart, msgEnd, section, sectionOffset);
        fflush(stderr);
        ((u8*)dest)[0] = 0xFD;
    }
#else
...
MessagePrintState* _msg_get_printer_for_msg(intptr_t msgID, bool* donePrintingWriteback, s32 arg2) {
    ...
    if (msgID >= 0) {
#endif
        srcBuffer = load_message_to_buffer((s32)msgID);
    }
...
#ifdef PORT
void get_msg_properties(intptr_t msgID, s32* height, s32* width, s32* maxLineChars, s32* numLines, s32* maxLinesPerPage, s32* numSpaces, u16 charset) {
#else
...
#if !VERSION_JP
#ifdef PORT
s32 get_msg_lines(intptr_t msgID) {
#else
