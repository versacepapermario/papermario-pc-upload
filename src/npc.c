    ...
    if (!isPeachNpc) {
        ...
        if (!(npc->flags & NPC_FLAG_HAS_NO_SPRITE)) {
            ...
            if (npc->spriteInstanceID < 0) {
                fprintf(stderr, "[create_npc_impl] WARNING: sprite load failed for NPC %d (animID=0x%X), disabling sprite\n",
                        i, npc->curAnim);
                npc->flags |= NPC_FLAG_HAS_NO_SPRITE | NPC_FLAG_INVISIBLE;
            }
#endif
        } else {
...
#ifdef PORT
                                    if (npc->spriteInstanceID < 0) {
                                        fprintf(stderr, "[update_npcs] WARNING: delayed sprite load failed (animID=0x%X), disabling sprite\n",
                                                npc->curAnim);
                                        npc->flags |= NPC_FLAG_HAS_NO_SPRITE | NPC_FLAG_INVISIBLE;
                                    } else {
                                        spr_update_sprite(npc->spriteInstanceID, npc->curAnim, npc->animationSpeed);
...
#ifdef PORT
        if (npc->spriteInstanceID < 0) {
            fprintf(stderr, "[set_npc_sprite] WARNING: sprite load failed (animID=0x%X), disabling sprite\n", anim);
            npc->flags |= NPC_FLAG_HAS_NO_SPRITE | NPC_FLAG_INVISIBLE;
        }
#else
        ASSERT(npc->spriteInstanceID >= 0);
...
#ifdef PORT
                    if (npc->spriteInstanceID < 0) {
                        fprintf(stderr, "[npc_reload_all] WARNING: sprite reload failed for NPC %d (animID=0x%X)\n",
                                i, npc->curAnim);
                        npc->flags |= NPC_FLAG_HAS_NO_SPRITE | NPC_FLAG_INVISIBLE;
                    }
#endif
...
u16 npc_blend_palette_colors(u16 colorA, u16 colorB, s32 lerpAlpha) {
    // Palette data from ROM is big-endian; byte-swap before extracting channels
    u16 nativeA = PAL_TO_NATIVE(colorA);
    u16 nativeB = PAL_TO_NATIVE(colorB);
    u8 r = (UNPACK_PAL_R(nativeA) * (255 - lerpAlpha) + UNPACK_PAL_R(nativeB) * lerpAlpha) / 255;
    u8 g = (UNPACK_PAL_G(nativeA) * (255 - lerpAlpha) + UNPACK_PAL_G(nativeB) * lerpAlpha) / 255;
    u8 b = (UNPACK_PAL_B(nativeA) * (255 - lerpAlpha) + UNPACK_PAL_B(nativeB) * lerpAlpha) / 255;
    u8 a = UNPACK_PAL_A(nativeB);

    return PAL_TO_BE(PACK_PAL_RGBA(r, g, b, a));
}
