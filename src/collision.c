...
#ifdef PORT
    // ROM data is big-endian — byte-swap the HitFile header (two u32 offsets)
    uncompressedData->collisionOffset = bswap32(uncompressedData->collisionOffset);
    uncompressedData->zoneOffset = bswap32(uncompressedData->zoneOffset);
#endif
...
#ifdef PORT
    // Byte-swap HitFileHeader fields from big-endian ROM data
    assetCollisionData->numColliders = bswap16(assetCollisionData->numColliders);
    assetCollisionData->collidersOffset = bswap32(assetCollisionData->collidersOffset);
    assetCollisionData->numVertices = bswap16(assetCollisionData->numVertices);
    assetCollisionData->verticesOffset = bswap32(assetCollisionData->verticesOffset);
    assetCollisionData->boundingBoxesDataSize = bswap16(assetCollisionData->boundingBoxesDataSize);
    assetCollisionData->boundingBoxesOffset = bswap32(assetCollisionData->boundingBoxesOffset);
#endif
...
#ifdef PORT
        *boundingBox = bswap32(*assetBoundingBox);
#else
...
#ifdef PORT
        vertices->x = (s16)bswap16(assetVertices->x);
        vertices->y = (s16)bswap16(assetVertices->y);
        vertices->z = (s16)bswap16(assetVertices->z);
#else
...
#ifdef PORT
        // Byte-swap HitAssetCollider fields
        assetCollider->boundingBoxOffset = bswap16(assetCollider->boundingBoxOffset);
        assetCollider->nextSibling = bswap16(assetCollider->nextSibling);
        assetCollider->firstChild = bswap16(assetCollider->firstChild);
        assetCollider->numTriangles = bswap16(assetCollider->numTriangles);
        assetCollider->trianglesOffset = bswap32(assetCollider->trianglesOffset);
#endif
        collider->flags = 0;
...
#ifdef PORT
                s32 tpVal = bswap32(*trianglePacked);
#else
                s32 tpVal = *trianglePacked;
#endif
                Vec3f* v1 = triangle->v1 = &collisionData->vertices[(tpVal) & 0x3FF];
                Vec3f* v2 = triangle->v2 = &collisionData->vertices[(tpVal >> 10) & 0x3FF];
                Vec3f* v3 = triangle->v3 = &collisionData->vertices[(tpVal >> 20) & 0x3FF];
                triangle->oneSided = (tpVal >> 30) & 1;
...
void parent_collider_to_model(s16 colliderID, s16 modelIndex) {
    ...
    // PORT FIX: buffer stores Vec3f* pointers, not Vec3f values.
    // On N64: 3*sizeof(Vec3f*) = 3*4 = 12 = sizeof(Vec3f). On 64-bit: 3*8 = 24 ≠ 12.
