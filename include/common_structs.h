...
// Color_RGBA8 and Color_RGB8 are provided by libultraship (ship/utils/color.h)
...
typedef struct Evt {
...
#ifdef PORT
    /* PORT: On 64-bit, s32[N] and void*[N] in a union have different element sizes,
     * causing per-index overlap bugs. Use intptr_t for all integer/pointer storage.
     * Float access: use *(f32*)&functionTemp[i] instead of functionTempF[i]. */
    /*       */ union {
    ...
    /* 0x0D0 */ intptr_t loopStartTable[8];
    /* 0x0F0 */ s32 loopCounterTable[8];
    /* 0x110 */ s8 switchBlockState[8];
    /* 0x118 */ s32 switchBlockValue[8];
    /* 0x138 */ Bytecode* buffer;
#ifdef PORT
    /* 0x13C */ Bytecode* array;  // intptr_t* on 64-bit to preserve pointer values
#else
    /* 0x13C */ s32* array;
#endif
    /* 0x140 */ s32* flagArray;
...
#ifdef PORT
// On PC (especially macOS), system headers may define 'Rect' with different fields.
// Use PM64_Rect to avoid the conflict.
typedef struct PM64_Rect {
    /* 0x00 */ s32 ulx;
    /* 0x04 */ s32 uly;
    /* 0x08 */ s32 lrx;
    /* 0x0C */ s32 lry;
} PM64_Rect; // size = 0x10
#define Rect PM64_Rect
#else
typedef struct Rect {
    /* 0x00 */ s32 ulx;
    /* 0x04 */ s32 uly;
