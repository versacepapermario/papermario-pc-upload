...
// Must be at least the width of a pointer (intptr_t) so that function pointer
// casts in EvtScript static initializers are compile-time constants on all platforms.
// On N64 (MIPS32), s32 == pointer width. On PC (64-bit), we need intptr_t.
// This mirrors Ghostship's approach: typedef uintptr_t BehaviorScript;
typedef intptr_t Bytecode;

// PORT: Helpers for accessing s32 data stored in Bytecode* arrays/buffers.
// On 64-bit, Bytecode* has 8-byte stride but data is s32 (4-byte elements).
// These helpers cast to s32* to use the correct stride.
static inline s32 port_array_get(Bytecode* array, s32 index) {
    return ((s32*)array)[index];
}
static inline void port_array_set(Bytecode* array, s32 index, s32 value) {
    ((s32*)array)[index] = value;
}
static inline s32 port_buf_peek(Bytecode* buf, s32 index) {
    return ((s32*)buf)[index];
}
static inline void port_buf_advance(Bytecode** bufPtr, s32 count) {
    *bufPtr = (Bytecode*)((s32*)*bufPtr + count);
}
