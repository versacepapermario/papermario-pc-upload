...
// On 64-bit, ModelNodePropertyData has void* (8 bytes) so each property is 0x10 instead of 0x0C.
// The bounding box overlay must match the actual ModelNodeProperty stride.
#ifdef PORT
// PORT: Each "row" is a ModelNodeProperty = {s32 key, s32 dataType, ModelNodePropertyData data}
// where data is 8 bytes (contains void*). So stride is 0x10 per property.
// Layout: prop[0]={key, halfSizeX, {.f=minX}}, prop[1]={unk, halfSizeY, {.f=minY}}, etc.
typedef struct ModelBoundingBox {
    s32 key;
    s32 halfSizeX;
    ModelNodePropertyData _data0; // .f = minX
    s32 _key1;
    s32 halfSizeY;
    ModelNodePropertyData _data1; // .f = minY
    s32 _key2;
    s32 halfSizeZ;
    ModelNodePropertyData _data2; // .f = minZ
    s32 _key3;
    s32 _dt3;
    ModelNodePropertyData _data3; // .f = maxX
    s32 _key4;
    s32 _dt4;
    ModelNodePropertyData _data4; // .f = maxY
    s32 _key5;
    s32 _dt5;
    ModelNodePropertyData _data5; // .f = maxZ
} ModelBoundingBox;
// Accessor macros for min/max since they're inside ModelNodePropertyData unions
#define BB_MIN_X(bb) ((bb)->_data0.f)
#define BB_MIN_Y(bb) ((bb)->_data1.f)
#define BB_MIN_Z(bb) ((bb)->_data2.f)
#define BB_MAX_X(bb) ((bb)->_data3.f)
#define BB_MAX_Y(bb) ((bb)->_data4.f)
#define BB_MAX_Z(bb) ((bb)->_data5.f)
#else
typedef struct ModelBoundingBox {
...
#define BB_MIN_X(bb) ((bb)->minX)
#define BB_MIN_Y(bb) ((bb)->minY)
#define BB_MIN_Z(bb) ((bb)->minZ)
#define BB_MAX_X(bb) ((bb)->maxX)
#define BB_MAX_Y(bb) ((bb)->maxY)
#define BB_MAX_Z(bb) ((bb)->maxZ)
#endif
...
#ifdef PORT
// On PC, shape files can be much larger than 32KB (e.g. KMR_02 is 0x199F0 = 104KB).
// On N64, gMapShapeData was at a fixed address (0x80210000) with plenty of space after it,
// but on PC the BSS allocation must be large enough for the actual data.
#define SHAPE_SIZE_LIMIT 0x80000
#else
