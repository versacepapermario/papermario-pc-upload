...
/**
 * macros.h - PC port version
 *
 * Changes from decomp:
 * - PHYSICAL_TO_VIRTUAL / VIRTUAL_TO_PHYSICAL → identity (no kseg0 mapping on PC)
 * - DMA_COPY_SEGMENT → no-op (data statically linked or loaded from O2R)
 * - BSS / SHIFT_BSS → standard storage (no linker section attributes needed)
 * - bool → use stdbool.h / C++ built-in
 * - Removed MIPS-specific __asm__ (NOP_FIX, NOP_UNFIX, macro.inc)
 * - Removed OLD_GCC workarounds
 * - FOLIAGE_MODEL_LIST uses __VA_ARGS__ instead of GNU extension
 */

// On PC, BSS is just regular storage - no special linker section needed
#define SHIFT_BSS extern
#define MATCHING_BSS(size)
#define BSS

// Apple Clang 17 doesn't support transparent_union for function parameters.
// Use empty definition; affected call sites use WS_ID() macro instead.
#define TRANSPARENT_UNION

// Helper macro for passing int style IDs to WindowStyle union parameters
#define WS_ID(id) ((WindowStyle){.defaultStyleID = (id)})
#define WS_CUSTOM(ptr) ((WindowStyle){.customStyle = (ptr)})

// Helper macros for WindowUpdateFunc transparent_union
#define WU_INT(val) ((WindowUpdateFunc){.i = (val)})
#define WU_FUNC(fn) ((WindowUpdateFunc){.func = (fn)})
...
// On PC, addresses are real pointers — no N64 kseg0 mapping needed
// These become identity operations (cast to correct type)
...
#ifdef GBI_FLOATS
// Float GBI: Mtx is MtxF with .mf field (float[4][4])
#define RDP_MATRIX(  \
    Ax, Bx, Cx, Dx, \
    Ay, By, Cy, Dy, \
    Az, Bz, Cz, Dz, \
...
#else
// Fixed-point GBI: Mtx is MtxS with .m field (int[4][4] packed)
#define RDP_MATRIX(  \
    Ax, Bx, Cx, Dx, \
    Ay, By, Cy, Dy, \
    Az, Bz, Cz, Dz, \
...
// PORT: Palette data from ROM is stored in big-endian (N64) format.
// On little-endian PC, byte-swap before CPU processing and after packing.
// Fast3D expects big-endian palette data so GPU-bound data stays as-is.
#ifdef PORT
#define PAL_TO_NATIVE(be_val) ((u16)((((u16)(be_val)) >> 8) | (((u16)(be_val)) << 8)))
#define PAL_TO_BE(native_val) ((u16)((((u16)(native_val)) >> 8) | (((u16)(native_val)) << 8)))
#else
#define PAL_TO_NATIVE(be_val) (be_val)
#define PAL_TO_BE(native_val) (native_val)
#endif
...
// On PC, VLA is always unsized (modern compilers support it)
#define VLA
...
// On PC, DMA_COPY_SEGMENT is a no-op — data is statically linked or loaded from O2R
// The original expands to: dma_copy(segment_ROM_START, segment_ROM_END, segment_VRAM)
#define DMA_COPY_SEGMENT(segment) ((void)0)

// Use standard bool from C23/C++
#ifndef __cplusplus
#include <stdbool.h>
#endif
