...
s32 draw_box(s32 flags, WindowStyle windowStyle, s32 posX, s32 posY, s32 posZ, s32 width, s32 height, u8 opacity,
              u8 darkening, f32 scaleX, f32 scaleY, f32 rotX, f32 rotY, f32 rotZ, void (*fpDrawContents)(intptr_t, s32, s32, s32, s32, s32, s32),
...
f32 sin_rad(f32 x);
f32 cos_rad(f32 x);
// Paper Mario's round() and atan2() have different signatures than the C standard library.
// They are renamed to pm_round() and pm_atan2() to avoid conflicts with math.h.
// The decomp source files use macros to redirect to these names.
s32 pm_round(f32);
...
// Forward declarations for AI types (defined in npc.h)
typedef struct MobileAISettings MobileAISettings;
typedef struct EnemyDetectVolume EnemyDetectVolume;
