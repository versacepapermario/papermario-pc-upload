...
static Interpreter* mInstanceRaw = nullptr;
...
const char* Interpreter::ACMUXtoStr(uint32_t acmux) {
    ...
    if (acmux >= tbl.size()) return "G_ACMUX_0";
    return tbl[acmux];
}
...
void Interpreter::GenerateCC(ColorCombiner* comb, const ColorCombinerKey& key) {
    ...
    if (!is2Cyc) {
        ...
        // In 1-cycle mode, N64 hardware maps TEXEL1 -> TEXEL0 (only one texture fetch)
        for (uint32_t k = 0; k < 4; k++) {
            if (c[0][0][k] == G_CCMUX_TEXEL1) c[0][0][k] = G_CCMUX_TEXEL0;
            if (c[0][0][k] == G_CCMUX_TEXEL1_ALPHA) c[0][0][k] = G_CCMUX_TEXEL0_ALPHA;
            if (c[0][1][k] == G_ACMUX_TEXEL1) c[0][1][k] = G_ACMUX_TEXEL0;
        }
    }
    {
                    ...
                    case G_CCMUX_1: // = G_CCMUX_CENTER = G_CCMUX_SCALE = 6
                        if (j == 1) {
                            ...
                            if (inputNumber[G_CCMUX_CENTER] == 0) {
                                ...
                                inputNumber[G_CCMUX_CENTER] = nextInputNumber++;
                            }
                            val = inputNumber[G_CCMUX_CENTER];
                        } else {
                            val = SHADER_1;
                        }
                        break;
...
const uint8_t* Interpreter::GetReplacementData(const RawTexMetadata* metadata, const uint8_t* fallback) {
    if (!metadata || !metadata->resource) return fallback;
...
void Interpreter::ImportTextureRgba16(int tile, bool importReplacement) {
    ...
    if (sizeBytes == 0) {
        return; // No texture data loaded for this tile
    }

    // LoadBlock: line_size_bytes == sizeBytes, use tile coordinates.
    // LoadTile: line_size_bytes is correct per-row byte count.
    uint32_t width, height;
    if (line_size_bytes == sizeBytes) {
        uint32_t uls = (uint32_t)mRdp->texture_tile[tile].uls >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrs = (uint32_t)mRdp->texture_tile[tile].lrs >> G_TEXTURE_IMAGE_FRAC;
        uint32_t ult = (uint32_t)mRdp->texture_tile[tile].ult >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrt = (uint32_t)mRdp->texture_tile[tile].lrt >> G_TEXTURE_IMAGE_FRAC;
        if (lrs < uls || lrt < ult || lrs - uls + 1 > 4096 || lrt - ult + 1 > 4096) {
            return;
        }
        width = lrs - uls + 1;
        height = lrt - ult + 1;
        ...
        height = sizeBytes / line_size_bytes;
...
void Interpreter::ImportTextureRgba32(int tile, bool importReplacement) {
    ...
    // LoadBlock: line_size_bytes == size_bytes, use tile coordinates.
    // LoadTile: line_size_bytes is correct per-row byte count.
    uint32_t width, height;
    if (line_size_bytes == size_bytes) {
        uint32_t uls = (uint32_t)mRdp->texture_tile[tile].uls >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrs = (uint32_t)mRdp->texture_tile[tile].lrs >> G_TEXTURE_IMAGE_FRAC;
        uint32_t ult = (uint32_t)mRdp->texture_tile[tile].ult >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrt = (uint32_t)mRdp->texture_tile[tile].lrt >> G_TEXTURE_IMAGE_FRAC;
        ...
        // RGBA32 is special: TMEM stores 2 bytes/pixel per bank (dual-bank architecture).
        // texture_tile.line_size_bytes reflects this 2-bpp TMEM bank line size,
        // while loaded_texture.line_size_bytes stores the full 4-bpp data width.
        // Use the TMEM bank line size for correct RGBA32 dimensions.
        if (mRdp->texture_tile[tile].line_size_bytes == 0) return;
        width = mRdp->texture_tile[tile].line_size_bytes / 2;
        height = (size_bytes / 2) / mRdp->texture_tile[tile].line_size_bytes;
    }
...
void Interpreter::ImportTextureIA4(int tile, bool importReplacement) {
    ...
    if (sizeBytes == 0 || sizeBytes > 4096) return;
    ...
    // Compute pixel dimensions for upload.
    // LoadBlock: lineSizeBytes == sizeBytes, use tile coordinates (TMEM line may have padding).
    // LoadTile: lineSizeBytes is correct per-row byte count, use directly.
    uint32_t width, height;
    if (lineSizeBytes == sizeBytes) {
        uint32_t uls = (uint32_t)mRdp->texture_tile[tile].uls >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrs = (uint32_t)mRdp->texture_tile[tile].lrs >> G_TEXTURE_IMAGE_FRAC;
        uint32_t ult = (uint32_t)mRdp->texture_tile[tile].ult >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrt = (uint32_t)mRdp->texture_tile[tile].lrt >> G_TEXTURE_IMAGE_FRAC;
...
void Interpreter::ImportTextureI4(int tile, bool importReplacement) {
    ...
    uint32_t width, height;
    if (lineSizeBytes == sizeBytes) {
        // LoadBlock: use tile coordinates for accurate dimensions
        uint32_t uls = (uint32_t)mRdp->texture_tile[tile].uls >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrs = (uint32_t)mRdp->texture_tile[tile].lrs >> G_TEXTURE_IMAGE_FRAC;
        uint32_t ult = (uint32_t)mRdp->texture_tile[tile].ult >> G_TEXTURE_IMAGE_FRAC;
        uint32_t lrt = (uint32_t)mRdp->texture_tile[tile].lrt >> G_TEXTURE_IMAGE_FRAC;
        ...
        if (lineSizeBytes == 0) return;
        width = lineSizeBytes * 2; // 2 pixels per byte for I4
        height = sizeBytes / lineSizeBytes;
    }
...
#if 0 // Diagnostic: I4 import logging (disabled — verified working)
    static int sI4LogCount = 0;
    if (sI4LogCount < 20) {
        fprintf(stderr, "[ImportI4] tile=%d width=%u height=%u sizeBytes=%u lineSizeBytes=%u fullImage=%u path=%s addr=%p\n",
            tile, width, height, sizeBytes, lineSizeBytes, fullImageLineSizeBytes,
            (lineSizeBytes == sizeBytes) ? "LoadBlock" : "LoadTile", (const void*)addr);
        sI4LogCount++;
    }
#endif
...
void Interpreter::ImportTextureI8(int tile, bool importReplacement) {
    ...
    if (sizeBytes == 0 || sizeBytes > 4096) {
        return; // No texture data loaded for this tile or corrupt size
    }
    ...
    // LoadBlock: lineSizeBytes == sizeBytes, use tile coordinates.
    // LoadTile: line_size_bytes is correct per-row byte count.
    uint32_t width, height;
    if (line_size_bytes == sizeBytes) {
        uint32_t uls = (uint32_t)mRdp->texture_tile[tile].uls >> G_TEXTURE_IMAGE_FRAC;
...
void Interpreter::ImportTextureCi4(int tile, bool importReplacement) {
    ...
    // When palettes[1] was loaded as an individual sub-palette (e.g., gDPLoadTLUT_pal16(pal=1)),
    // it points directly to that sub-palette's data — not offset within a contiguous TLUT.
    // Use it directly when the requested palIdx matches the individually-loaded index.
    if (palIdx > 0 && palIdx < 8 && mRdp->palette1_pal_index == (int8_t)palIdx) {
        palette = mRdp->palettes[1];
    } else if (palIdx > 7) {
    ...
    // Handle subtile case: iterate row-by-row with proper stride.
    // For CI4, each source byte = 2 pixels (4 bits each).
    // lineSizeBytes = tile row width in bytes, fullImageLineSizeBytes = full image row width.
    if (lineSizeBytes == 0 || sizeBytes > 4096) return;
    uint32_t tileHeight = sizeBytes / lineSizeBytes;
    uint32_t outPixel = 0;
    for (uint32_t row = 0; row < tileHeight; row++) {
        for (uint32_t col = 0; col < lineSizeBytes; col++) {
            uint8_t byte = addr[row * fullImageLineSizeBytes + col];
            // High nibble (first pixel)
            uint8_t idx = (byte >> 4) & 0xf;
            uint16_t col16 = (palette[idx * 2] << 8) | palette[idx * 2 + 1]; // Big endian load
            mTexUploadBuffer[4 * outPixel + 0] = SCALE_5_8(col16 >> 11);
            mTexUploadBuffer[4 * outPixel + 1] = SCALE_5_8((col16 >> 6) & 0x1f);
            mTexUploadBuffer[4 * outPixel + 2] = SCALE_5_8((col16 >> 1) & 0x1f);
            mTexUploadBuffer[4 * outPixel + 3] = (col16 & 1) ? 255 : 0;
            outPixel++;
    ...
    // Compute pixel dimensions for upload.
    // LoadBlock path: lineSizeBytes == sizeBytes (entire texture as one "line"), so the tile's
    // TMEM line field may have 8-byte padding for narrow 4-bit textures. Use tile coordinates
    // from gDPSetTileSize which has the actual pixel dimensions.
    // LoadTile path: lineSizeBytes is the correct per-row byte count from LoadTile coordinates.
    // Use it directly since gDPSetTileSize may not have been called for this tile.
    uint32_t width, height;
    if (lineSizeBytes == sizeBytes) {
        // LoadBlock: use tile coordinates for accurate dimensions
        uint32_t uls = (uint32_t)mRdp->texture_tile[tile].uls >> G_TEXTURE_IMAGE_FRAC;
        ...
        width = lineSizeBytes * 2; // 2 pixels per byte for CI4
        height = sizeBytes / lineSizeBytes;
    }

    if (metadata->h_byte_scale != 1) {
        width *= metadata->h_byte_scale;
    }
    if (metadata->v_pixel_scale != 1) {
        height *= metadata->v_pixel_scale;
    }
...
void Interpreter::ImportTextureCi8(int tile, bool importReplacement) {
    ...
    if (sizeBytes == 0 || sizeBytes > 4096 || lineSizeBytes == 0) return;
    ...
    if (lineSizeBytes == sizeBytes) {
        ...
        if (resultLineSizeBytes == 0) return;
        width = resultLineSizeBytes;
        height = sizeBytes / resultLineSizeBytes;
    }
...
void Interpreter::ImportTexture(int i, int tile, bool importReplacement) {
    ...
    TextureCacheKey key;
    if (fmt == G_IM_FMT_CI) {
        // Hash palette AND texture content for CI textures.
        // Data may change in-place while the pointer stays the same
        // (e.g., storybook pages DMA'd into the same buffer, SpriteShadingPalette).
        uint32_t pal_hash = 0;
        if (mRdp->palette1_pal_index >= 0 &&
            mRdp->palette1_pal_index == (int8_t)paletteIndex &&
            mRdp->palettes[1] != nullptr) {
            // Sub-palette (16 entries = 32 bytes)
            const uint8_t* p = mRdp->palettes[1];
            for (int j = 0; j < 32; j++) {
                pal_hash = pal_hash * 31 + p[j];
            }
        } else if (mRdp->palettes[0] != nullptr) {
            // Full 256-entry palette — hash all 512 bytes for reliable detection
            const uint8_t* p = mRdp->palettes[0];
            for (int j = 0; j < 512; j++) {
                pal_hash = pal_hash * 31 + p[j];
            }
        }
        // Hash texture data to detect in-place overwrites (e.g., storybook page flips,
        // player sprite raster cache reusing the same buffer for different rasters).
        // For small textures (<=4KB), hash every byte to catch subtle differences
        // (e.g., Peach blink rasters differ by only ~9 bytes out of 1728).
        // For larger textures, sample 256 bytes spread across the data.
        if (origAddr != nullptr && origSizeBytes > 0) {
            if (origSizeBytes <= 4096) {
                for (uint32_t j = 0; j < origSizeBytes; j++) {
                    pal_hash = pal_hash * 31 + origAddr[j];
                }
            } else {
                uint32_t hashBytes = 256;
                uint32_t stride = origSizeBytes / hashBytes;
                if (stride < 1) stride = 1;
                for (uint32_t j = 0; j < origSizeBytes && hashBytes > 0; j += stride, hashBytes--) {
                    pal_hash = pal_hash * 31 + origAddr[j];
                }
            }
        }
        key = { origAddr, { mRdp->palettes[0], mRdp->palettes[1] }, fmt, siz, paletteIndex, origSizeBytes, pal_hash };
    } else {
        // For I/IA textures that may be dynamically updated (e.g. flame effect writes
        // to D_800B32E0 each frame), hash the content to detect changes.
        // Without this, the address-only cache key returns stale textures.
        uint32_t content_hash = 0;
        if ((fmt == G_IM_FMT_I || fmt == G_IM_FMT_IA) && origAddr != nullptr && origSizeBytes > 0 && origSizeBytes <= 4096) {
            for (uint32_t j = 0; j < origSizeBytes; j += 4) {
                content_hash = content_hash * 31 + origAddr[j];
            }
        }
        key = { origAddr, {}, fmt, siz, paletteIndex, origSizeBytes, content_hash };
    }
...
float Interpreter::AdjXForAspectRatio(float x) const {
    if (mFbActive) {
        ...
        if (mCurDimensions.height == 0) return x;
...
void Interpreter::GfxSpVertex(size_t n_vertices, size_t dest_index, const F3DVtx* vertices) {
    ...
    // Clamp to prevent out-of-bounds access on loaded_vertices[MAX_VERTICES + 4]
    if (dest_index >= MAX_VERTICES || n_vertices > MAX_VERTICES || dest_index + n_vertices > MAX_VERTICES) {
        return;
    }
...
void Interpreter::GfxSpModifyVertex(uint16_t vtx_idx, uint8_t where, uint32_t val) {
    ...
    switch (where) {
        case G_MWO_POINT_RGBA:
            v->color.r = (val >> 24) & 0xFF;
            v->color.g = (val >> 16) & 0xFF;
            v->color.b = (val >> 8) & 0xFF;
            v->color.a = val & 0xFF;
            break;
        case G_MWO_POINT_ST:
            v->u = (int16_t)(val >> 16);
        ...
        case G_MWO_POINT_XYSCREEN: {
            // Screen coordinates in 10.2 fixed point format
            int16_t sx = (int16_t)(val >> 16);
            int16_t sy = (int16_t)val;
            float halfW = HALF_SCREEN_WIDTH(mActiveFrameBuffer);
            float halfH = HALF_SCREEN_HEIGHT(mActiveFrameBuffer);
            v->x = (sx / 4.0f) / halfW - 1.0f;
            v->y = -((sy / 4.0f) / halfH) + 1.0f;
            v->w = 1.0f;
            break;
        }
        case G_MWO_POINT_ZSCREEN: {
            // Z value in screen space (16.16 fixed point: upper 16 = z, lower 16 = dz)
            // Just ignore for now — depth modification rarely matters for visual correctness
            break;
        }
        default:
            break;
...
void Interpreter::GfxSpTri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    if (vtx1_idx > maxIdx || vtx2_idx > maxIdx || vtx3_idx > maxIdx) {
        return;
    }
    ...
    // Guard against vertices behind or near the camera plane.
    // Vertices with very small w produce extreme NDC after perspective division,
    // creating thin triangles that stretch across the screen ("random lines").
    // Also discard triangles with garbage vertex data (astronomically large values).
    ...
    // N64 RDP: depth comparison requires BOTH G_ZBUFFER (RSP generates Z) AND
    // Z_CMP (RDP compares against Z-buffer). Effects like damage numbers set
    // render modes without Z_CMP to always draw on top of 3D geometry.
    bool depth_test = (mRsp->geometry_mode & G_ZBUFFER) && (mRdp->other_mode_l & Z_CMP);
    ...
    // Enable alpha blending when the blender reads CLR_MEM with 1MA (one minus alpha) factor,
    // OR with G_BL_1 factor (additive blending: P*A + M*1, used by glow/fire effects).
    // Note: G_BL_A_MEM (coverage alpha) must NOT be included here — it's used by all
    // anti-aliased opaque modes (AA_ZB_OPA_SURF, etc.) and enabling alpha for those
    // makes opaque geometry translucent. CLD_SURF uses 1MA, not A_MEM.
    // Additive blend is approximated as standard alpha blend on PC (src*a + dst*(1-a)).
    bool use_alpha = ((mRdp->other_mode_l & (3 << 20)) == (G_BL_CLR_MEM << 20) &&
                      ((mRdp->other_mode_l & (3 << 16)) == (G_BL_1MA << 16) ||
                       (mRdp->other_mode_l & (3 << 16)) == (G_BL_1 << 16))) ||
                     ((mRdp->other_mode_l & (3 << 22)) == (G_BL_CLR_MEM << 22) &&
                      ((mRdp->other_mode_l & (3 << 18)) == (G_BL_1MA << 18) ||
                       (mRdp->other_mode_l & (3 << 18)) == (G_BL_1 << 18)));
    ...
    if (texture_edge) {
        if (use_alpha) {
            // Standard alpha blend + CVG_X_ALPHA = use alpha threshold for edge cutoff
            alpha_threshold = true;
            texture_edge = false;
        } else {
            // CVG_X_ALPHA without standard alpha blend.
            uint32_t p1 = (mRdp->other_mode_l >> 30) & 3;
            uint32_t p2 = (mRdp->other_mode_l >> 28) & 3;
            if (p1 != G_BL_CLR_IN && p2 != G_BL_CLR_IN) {
                // P≠CLR_IN: framebuffer self-blend (e.g. CLR_MEM*A + CLR_MEM*A_MEM).
                // Coverage-only effect — make invisible since actual content renders
                // via separate DLs with standard alpha blending.
                texture_edge = false;
                invisible = true;
            }
            // else: P=CLR_IN — standard opaque AA surface, keep texture_edge
        }
        use_alpha = true;
    }
    ...
    for (int i = 0; i < 2; i++) {
        uint32_t tile = (mRdp->first_tile_index + i) & 7;
        if (comb->usedTextures[i]) {
            ...
            // Fix for non-power-of-2 width textures: The TMEM line field rounds up
            // to 8-byte alignment, so texture_tile.line_size_bytes can be larger than
            // the actual pixel row width. Use loaded_texture.line_size_bytes from LoadTile
            // which has the actual per-row byte count, matching what Import functions use.
            ...
            if (tex_height[i] == 0) tex_height[i] = 1;

            float tw2 = (mRdp->texture_tile[tile].lrs - mRdp->texture_tile[tile].uls + 4) / 4;
            float th2 = (mRdp->texture_tile[tile].lrt - mRdp->texture_tile[tile].ult + 4) / 4;
            tex_width2[i] = tw2 > 0 ? (uint32_t)tw2 : 1;
            tex_height2[i] = th2 > 0 ? (uint32_t)th2 : 1;
    ...
    // Update alpha compare threshold from blend_color.a (used by G_AC_THRESHOLD shaders)
    mRapi->mAlphaCompareThreshold = mRdp->blend_color.a / 255.0f;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // G_ZS_PRIM: use primitive depth instead of per-vertex depth
        if (mRdp->other_mode_l & G_ZS_PRIM) {
            // N64 prim depth z is 15-bit: 0 = nearest, 0x7FFF = farthest
            // Normalize to [0, 1] then map to NDC [-1, 1] range
            float prim_z_normalized = (mRdp->prim_depth_z & 0x7FFF) / 32768.0f;
            z = (2.0f * prim_z_normalized - 1.0f) * w;
        }
        ...
        for (int t = 0; t < 2; t++) {
            ...
            int shifts = mRdp->texture_tile[(mRdp->first_tile_index + t) & 7].shifts;
            int shiftt = mRdp->texture_tile[(mRdp->first_tile_index + t) & 7].shiftt;
            if (shifts != 0) {
                if (shifts <= 10) {
                    u /= 1 << shifts;
                } else {
            ...
            u -= mRdp->texture_tile[(mRdp->first_tile_index + t) & 7].uls / 4.0f;
            v -= mRdp->texture_tile[(mRdp->first_tile_index + t) & 7].ult / 4.0f;
        ...
        for (int j = 0; j < numInputs; j++) {
            ...
            for (int k = 0; k < 1 + (use_alpha ? 1 : 0); k++) {
                switch (comb->shader_input_mapping[k][j]) {
                    ...
                    case G_CCMUX_LOD_FRACTION: {
                        // Use prim_lod_fraction as approximation for LOD_FRACTION.
                        // The N64 RDP computes this from texture coordinate magnification,
                        // but prim_lod_fraction is set by the game and works for PM64's
                        // mipmapped textures which use G_TL_LOD.
                        tmp.r = tmp.g = tmp.b = tmp.a = mRdp->prim_lod_fraction;
                        color = &tmp;
                        break;
                    }
...
void Interpreter::GfxSpMovewordF3dex2(uint8_t index, uint16_t offset, uintptr_t data) {
    switch (index) {
        ...
        case G_MW_SEGMENT_INTERP: {
            ...
            if (segNumber >= 0 && segNumber < (int)MAX_SEGMENT_POINTERS && segIndex == mInterpolationIndex)
                mSegmentPointers[segNumber] = data;
        } break;
    }
}
...
void Interpreter::GfxDpSetTile(uint8_t fmt, uint32_t siz, uint32_t line, uint32_t tmem, uint8_t tile, uint32_t palette,
    ...
    // Handle split-tile patterns (e.g., gDPScrollTextureBlockHalfHeight) where a single
    // LoadBlock at TMEM 0 is divided into two render tiles at different TMEM offsets.
    // When a render tile's TMEM offset falls within loaded_texture[0]'s range and no
    // separate load was done for loaded_texture[1], derive it with the correct byte offset.
    if (tmem != 0 && tile != G_TX_LOADTILE &&
        mRdp->tex_loaded_at[1] < mRdp->tex_loaded_at[0] &&
        mRdp->loaded_texture[0].addr != nullptr) {
        uint32_t byte_offset = (uint32_t)tmem * 8;
        if (byte_offset < mRdp->loaded_texture[0].orig_size_bytes) {
            mRdp->loaded_texture[1] = mRdp->loaded_texture[0];
            mRdp->loaded_texture[1].addr += byte_offset;
            mRdp->loaded_texture[1].size_bytes -= byte_offset;
            mRdp->loaded_texture[1].orig_size_bytes -= byte_offset;
            mRdp->loaded_texture[1].line_size_bytes = mRdp->loaded_texture[1].size_bytes;
            mRdp->loaded_texture[1].full_image_line_size_bytes = mRdp->loaded_texture[1].size_bytes;
            mRdp->textures_changed[1] = true;
        }
    }
...
void Interpreter::GfxDpLoadTlut(uint8_t tile, uint32_t high_index) {
    ...
    uint32_t tmem_offset = mRdp->texture_tile[tile].tmem;

    if (tmem_offset == 256) {
        mRdp->palettes[0] = mRdp->texture_to_load.addr;
        if (high_index == 255) {
            // Full 256-entry TLUT load — sub-palettes 0-7 contiguous at palettes[0],
            // sub-palettes 8-15 contiguous at palettes[1]
            mRdp->palettes[1] = mRdp->texture_to_load.addr + 2 * 128;
            mRdp->palette1_pal_index = -1; // Contiguous layout
        }
    } else {
        // Individual sub-palette load (pal 1-15)
        // gDPLoadTLUT_pal16 sets tmem = 256 + pal_index * 16
        int pal_index = (tmem_offset - 256) / 16;
        mRdp->palettes[1] = mRdp->texture_to_load.addr;
        mRdp->palette1_pal_index = (int8_t)pal_index;
    }
}
...
void Interpreter::GfxDpLoadBlock(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t dxt) {
    ...
    mRdp->tex_load_counter++;
    mRdp->tex_loaded_at[mRdp->texture_tile[tile].tmem_index] = mRdp->tex_load_counter;
}
...
void Interpreter::GfxDrawRectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry) {
    ...
    // G_ZS_PRIM: use primitive depth for rectangles (needed for Z-buffer tricks)
    float rect_z = -1.0f;
    if (mRdp->other_mode_l & G_ZS_PRIM) {
        // N64 prim depth z is 15-bit: 0 = nearest, 0x7FFF = farthest
        float prim_z_normalized = (mRdp->prim_depth_z & 0x7FFF) / 32768.0f;
        rect_z = 2.0f * prim_z_normalized - 1.0f;
    }
    ...
    mRdp->viewport_or_scissor_changed = true;
    // Preserve G_ZBUFFER so rectangles can participate in Z-testing
    // (needed for G_ZS_PRIM overlays like the battle transition star)
    mRsp->geometry_mode = geometry_mode_saved & G_ZBUFFER;
...
bool gfx_dl_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    if (C0(16, 1) == 0 && nDL != nullptr) {
        ...
        if (nDL != nullptr) {
            ...
            SPDLOG_WARN("OTR DL filepath handler: resource not found, skipping");
        }
    }
    return false;
}
...
bool gfx_dl_handler_common(F3DGfx** cmd0) {
    ...
    // On 64-bit, reject sub-DL pointers below 4GB (likely unconverted N64 addresses)
    if (sizeof(void*) > 4 && subGFX != nullptr && (uintptr_t)subGFX < 0x100000000ULL) {
        subGFX = nullptr;
    }
    // Validate sub-DL first command: reject garbage data
    if (subGFX != nullptr) {
        // Opcode is at bits 24-31 of w0 (same as N64 GBI convention)
        uint8_t firstOp = (uint8_t)(subGFX->words.w0 >> 24);
        // On PC, the upper 32 bits of w0 should always be 0 (GBI commands are 32-bit)
        uint32_t upperW0 = (uint32_t)(subGFX->words.w0 >> 32);
        bool likelyGarbage = false;
        if (upperW0 != 0) {
            // Upper 32 bits non-zero — this is not a valid PC Gfx command
            likelyGarbage = true;
        } else if (firstOp >= 0x08 && firstOp <= 0xCF) {
            // Valid F3DEX2 opcodes: 0x00-0x07 (geometry), 0xD3-0xFF (RDP/high)
            likelyGarbage = true;
        }
        if (likelyGarbage) {
            static int sBadDlCount = 0;
            if (sBadDlCount < 20) {
                // Dump the full DL call path for diagnosis
                auto& path = g_exec_stack.gfx_path;
                std::string pathStr;
                for (size_t pi = 0; pi < path.size(); pi++) {
                    char buf[64];
                    snprintf(buf, sizeof(buf), "%s%p", pi ? " -> " : "", (void*)path[pi]);
                    pathStr += buf;
                }
                SPDLOG_WARN("G_DL: skipping sub-DL at {:p} — first cmd w0=0x{:016X} looks like garbage "
                            "(caller at {:p}, caller w0=0x{:016X} w1=0x{:016X}, stack depth {}, path: {})",
                            (void*)subGFX, (uint64_t)subGFX->words.w0, (void*)cmd,
                            (uint64_t)cmd->words.w0, (uint64_t)cmd->words.w1,
                            g_exec_stack.cmd_stack.size(), pathStr);
                sBadDlCount++;
            }
            subGFX = nullptr;
        }
    ...
    if (C0(16, 1) == 0) {
        ...
        // Branch mode — must not branch to NULL
        if (subGFX != nullptr) {
            (*cmd0) = subGFX;
            g_exec_stack.branch(cmd);
            return true; // shortcut cmd increment
...
bool gfx_set_prim_depth_handler_rdp(F3DGfx** cmd) {
    ...
    gfx->mRdp->prim_depth_z = (uint16_t)(w1 >> 16);
    gfx->mRdp->prim_depth_dz = (uint16_t)(w1 & 0xFFFF);
    return false;
}
...
bool gfx_set_timg_handler_rdp(F3DGfx** cmd0) {
    ...
    // On 64-bit, redirect unconverted N64 VRAM addresses (below 4GB) to a dummy
    // texture buffer. These are raw ROM/VRAM pointers that haven't been rebased
    // to PC addresses and would crash if dereferenced.
    if (i != 0 && i < 0x100000000ULL) {
        static uint8_t sDummyTexture[4096] = {};
        static bool sLoggedOnce = false;
        if (!sLoggedOnce) {
            SPDLOG_WARN("G_SETTIMG: redirecting unconverted N64 address 0x{:X} to dummy texture", i);
            sLoggedOnce = true;
        }
        i = (uintptr_t)sDummyTexture;
    }
#endif
...
bool gfx_set_key_r_handler_rdp(F3DGfx** cmd0) {
    ...
    gfx->GfxDpSetKeyR((uint32_t)cmd->words.w1);
    return false;
}
...
bool gfx_set_key_gb_handler_rdp(F3DGfx** cmd0) {
    ...
    gfx->GfxDpSetKeyGB((uint32_t)cmd->words.w1);
    return false;
}
...
static void gfx_step() {
    ...
    // Guard: on 64-bit PC, valid Gfx commands always have upper 32 bits of w0 = 0.
    // Non-zero upper bits means we're executing non-DL data (vertices, matrices, etc.)
    // Abort back to the parent DL immediately.
    {
        uint32_t upperW0 = (uint32_t)(cmd->words.w0 >> 32);
        if (upperW0 != 0) {
            static int sGarbageAbortCount = 0;
            if (sGarbageAbortCount < 5) {
                SPDLOG_WARN("Aborting garbage DL at {:p}: w0=0x{:016X} w1=0x{:016X}, stack depth {}",
                            (void*)cmd, (uint64_t)cmd->words.w0, (uint64_t)cmd->words.w1,
                            g_exec_stack.cmd_stack.size());
                // Log parent DL call chain
                for (size_t pi = 0; pi < g_exec_stack.gfx_path.size(); pi++) {
                    auto* p = g_exec_stack.gfx_path[pi];
                    if (p) {
                        SPDLOG_WARN("  gfx_path[{}]: {:p} w0=0x{:016X} w1=0x{:016X}",
                                    pi, (void*)p, (uint64_t)p->words.w0, (uint64_t)p->words.w1);
                    }
                }
                sGarbageAbortCount++;
            }
            if (g_exec_stack.cmd_stack.size() > 1) {
                g_exec_stack.ret();
                return;
            }
            ++cmd;
...
#define TRACE                                  \
    ...
    // Guard: if ucode_handler_index was corrupted (e.g. by memory corruption),
    // reset it to f3dex2 so we can continue processing valid commands.
    if (ucode_handler_index >= ucode_max) {
        static int sUcodeResetCount = 0;
        if (sUcodeResetCount < 5) {
            SPDLOG_CRITICAL("ucode_handler_index corrupted to {} — resetting to f3dex2 (opcode=0x{:02X} w0=0x{:016X} w1=0x{:016X})",
                            (uint32_t)ucode_handler_index, (uint8_t)opcode, (uint64_t)cmd->words.w0, (uint64_t)cmd->words.w1);
            sUcodeResetCount++;
        }
        ucode_handler_index = ucode_f3dex2;
    }

    if (opcode == F3DEX2_G_LOAD_UCODE) {
        UcodeHandlers requested = (UcodeHandlers)(cmd->words.w0 & 0xFFFFFF);
        if (requested < ucode_max) {
            gfx_set_ucode_handler(requested);
        } else {
            SPDLOG_CRITICAL("G_LOAD_UCODE: rejected invalid ucode {} (w0=0x{:016X} w1=0x{:016X}), keeping current ucode {}",
                            (uint32_t)requested, (uint64_t)cmd->words.w0, (uint64_t)cmd->words.w1, (uint32_t)ucode_handler_index);
        }
        ++cmd;
        return;
    ...
    // Track unhandled opcodes — if we see too many, we're reading garbage data
    // and should abort the current display list.
    static int sUnhandledCount = 0;
    static int sTotalCmdCount = 0;
    sTotalCmdCount++;
    bool handled = false;

    // OTR handlers use game-specific opcodes (0x20-0x40 range) for OTR resource
    // loading. Games that load assets from ROM at runtime (not from OTR archives)
    // should skip these — any match is from garbage data being misinterpreted.
    // Check the PAPERSHIP_SKIP_OTR_HANDLERS CVar or compile-time flag.
#ifndef SKIP_OTR_HANDLERS
    if (otrHandlers.contains(opcode)) {
        handled = true;
        sUnhandledCount = 0;
    ...
    if (!handled) {
        sUnhandledCount++;
        if (sUnhandledCount > 5) {
            // Too many unhandled opcodes — abort this display list (likely garbage data)
            if (sUnhandledCount == 6) {
                SPDLOG_WARN("Aborting garbage display list after {} unhandled opcodes at DL addr {:p}, stack depth {}",
                            sUnhandledCount, (void*)cmd, g_exec_stack.cmd_stack.size());
            }
            // Return to parent display list
            if (g_exec_stack.cmd_stack.size() > 1) {
                g_exec_stack.ret();
                sUnhandledCount = 0;
                return; // Don't touch cmd — it references the popped entry
            }
            // If we're at the top-level DL, just skip
            ++cmd;
            return;
        }
        SPDLOG_CRITICAL("Unhandled OP code: 0x{:X} w0=0x{:016X} w1=0x{:016X} at DL addr {:p} (stack depth {}, ucode {})",
                        (uint8_t)opcode, (uint64_t)cmd->words.w0, (uint64_t)cmd->words.w1,
                        (void*)cmd, g_exec_stack.cmd_stack.size(), (uint32_t)ucode_handler_index);
    }
...
void Interpreter::RunGuiOnly() {
    ...
    // PORT: Clear color buffer to black. N64 framebuffer retains black from prior frames,
    // but on PC the GPU default is white, causing visible white strips in areas without
    // explicit background fills (e.g. underground maps with no background image).
...
int32_t gfx_check_image_signature(const char* imgData) {
...
#if UINTPTR_MAX > 0xFFFFFFFFu
    // On 64-bit platforms, reject pointers in the lower 4GB address range.
    // These are unconverted N64 VRAM addresses (0x80XXXXXX) that would crash
    // when dereferenced. Valid PC pointers on 64-bit are always above 4GB.
    if (i != 0 && i < 0x100000000ULL) {
        return 0;
    }
#endif
    ...
    if (i != 0) {
#ifdef NO_OTR_TEXTURES
        // Game does not use OTR texture resources — skip signature check to avoid
        // dereferencing arbitrary texture data pointers (which can crash if the
        // memory has been freed during map transitions, etc.)
        return 0;
#else
...
extern "C" int gfx_create_framebuffer(uint32_t width, uint32_t height, uint32_t native_width, uint32_t native_height,
                                      uint8_t resize) {
    Fast::Interpreter* gfx = Fast::mInstanceRaw;
    if (!gfx) return -1;
    return gfx->CreateFrameBuffer(width, height, native_width, native_height, resize);
}
...
extern "C" void gfx_copy_framebuffer(int fb_dst, int fb_src) {
    Fast::Interpreter* gfx = Fast::mInstanceRaw;
    if (!gfx) return;
    gfx->Flush();
    gfx->CopyFrameBuffer(fb_dst, fb_src, false, nullptr);
}

extern "C" void gfx_read_framebuffer_to_cpu(int fb_id, uint32_t width, uint32_t height, uint16_t* rgba16_buf) {
    Fast::Interpreter* gfx = Fast::mInstanceRaw;
    if (!gfx) return;
    gfx->mRapi->ReadFramebufferToCPU(fb_id, width, height, rgba16_buf);
}
