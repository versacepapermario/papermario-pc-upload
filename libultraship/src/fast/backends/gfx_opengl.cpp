...
void GfxRenderingAPIOGL::SetPerDrawUniforms() {
    ...
    if (mCurrentShaderProgram->blend_color_alpha_location >= 0) {
        glUniform1f(mCurrentShaderProgram->blend_color_alpha_location, mAlphaCompareThreshold);
    }
}
...
ShaderProgram* GfxRenderingAPIOGL::CreateAndLoadNewShader(uint64_t shader_id0, uint32_t shader_id1) {
    ...
    prg->blend_color_alpha_location = glGetUniformLocation(shader_program, "blend_color_alpha");
