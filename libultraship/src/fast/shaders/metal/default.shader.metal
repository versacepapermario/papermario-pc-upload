...
@end
    ...
    @if(o_texture_edge && o_alpha)
        @if(o_alpha_threshold)
            if (texel.w > drawUniforms.blendColorAlpha) texel.w = 1.0; else discard_fragment();
        @else
            if (texel.w > drawUniforms.blendColorAlpha) texel.w = 1.0; else discard_fragment();
        @end
    @end
    ...
    @if(o_alpha)
        @if(o_alpha_threshold && !o_texture_edge)
            if (texel.w < drawUniforms.blendColorAlpha) discard_fragment();
        @end
        @if(o_invisible)
            texel.w = 0.0;
        @end
