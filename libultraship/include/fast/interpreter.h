...
struct TextureCacheKey {
    ...
    uint32_t size_bytes;
    uint32_t palette_content_hash; // Non-zero when palette contents may change in-place
...
struct RDP {
    ...
    int8_t palette1_pal_index; // Sub-palette index that palettes[1] was loaded for (-1 = full TLUT second half)
    struct {
        const uint8_t* addr;
        uint8_t siz;
        uint32_t width;
    ...
    uint32_t tex_load_counter;
    uint32_t tex_loaded_at[2]; // generation stamp per loaded_texture slot
    ...
    uint16_t prim_depth_z;     // G_SETPRIMDEPTH z value
    uint16_t prim_depth_dz;    // G_SETPRIMDEPTH delta z
    struct RGBA env_color, prim_color, fog_color, fill_color, grayscale_color, blend_color, key_center;
...
class Interpreter {
  public:
    ...
    const uint8_t* GetReplacementData(const RawTexMetadata* metadata, const uint8_t* fallback);
    ...
    void GfxDpSetKeyR(uint32_t w1);
    void GfxDpSetKeyGB(uint32_t w1);
